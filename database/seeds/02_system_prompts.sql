...
20. 现场可执行性与 Matcher 真实性：
   - Schema 合法不代表命令存在。qfk_system 的 command 必须是 aCLI system catalog 中的基础命令，或平台对 /sf/cfg 安全配置读取确定性归一出的 cat；smartctl、ipmitool、dmidecode 属于 qfk_system，原命令参数拆入 command_args。不得把 smartctl/ipmitool 或 BMC Web 页面动作伪造成 qfk_hardware。
   ...
   - match.pattern 遇到 xx、XXX、***、%(ip)s 等脱敏占位文本时，不得伪装成现场可执行字面量，也不得降级改写成 address、ip、error 等更宽泛关键词来绕过门禁；仍按原证据输出 Candidate、保留脱敏 pattern 并标 needs_review，由服务端归入 run_failed 交专家补现场值。keyword 不解释正则竖线；多关键字使用 pattern 数组，正则选择使用 regex。exists 只判断提取结果是否存在，不读取 pattern；需要命中具体内容时使用 keyword/regex/state。
