...
5. backend（qfk_*）执行模式严格二选一：普通 aCLI QFK 判定模式配置 match 且 produces=[]；产出变量模式配置 match=null 且 produces 非空。frontend（qkv_*）信号可省略 match 或置 null。判定模式的 match 必须包含 extract，Matcher 与 produces 复用同一份安全转换管道。match.type 仅允许 [keyword, regex, state, threshold, delta, trend, exists]；JSON 路径属于 extract.type=json 的取值方式，不是 Matcher 类型；匹配关键词放 match.pattern（不要放 acquire.args）。**例外：qfk_var 是纯变量处理器，不生成 aCLI 命令；assert 的 compare/exists 规则写在 acquire.args 并要求 match=null、produces=[]；derive 要求 match=null 且 produces 恰好一个。**
6. acquire.args 字段对照（关键，务必对齐，多/错字段会被契约拒绝）：
   ...
   - qfk_var：变量池处理器。必须显式声明 orchestrate.requires，输入只允许引用这些变量；operation 白名单为 feature_extract/field/json_path/cast/string/compose/compare/exists。feature_extract 固定执行“确定性变量提取 → 变量命名和归一化 → 类型和基数校验 → AI 兜底”四层流水线；普通编辑人员只选择 target_variable，不能手写通用正则。AI 仅在无稳定边界/新格式时启用，稳定多候选必须返回 ambiguous，不得调用 AI；AI 值必须逐字存在于输入证据。
   - qfk_system：command 只写基础子命令（如 lsof/ps/cat/grep），普通参数唯一写入 command_args 数组（如 ["-p","{{{{PID}}}}","-o","cmd="]）；可选 host（{{{{HOST}}}}）/timeout/instruction/nonzero_exit_as_negative。qfk_system 禁止 resource_keyword，VM ID 必须放在 produces.extract.rows.include 的受控行筛选中，不能追加给 lsof。
   - 只读探针容错（nonzero_exit_as_negative）：当 qfk_* 命令为只读探测（如 cat、grep、pgrep 等）且探测对象可能在目标机缺失（导致 POSIX exit 1）时，可在 acquire.args 中显式指定 nonzero_exit_as_negative=true，使对象不存在被正确消费为否定事实(matched=False)完成排除，而非系统执行故障。写操作严禁使用。
   - qfk_vm/network/storage/hardware/platform：command（如 list/show/...，acli <namespace> <command>）；可选 host（{{{{HOST}}}}）/resource_keyword/timeout/instruction/nonzero_exit_as_negative。
...
20. 现场可执行性与 Matcher 真实性：
   ...
   - match.pattern 遇到 xx、XXX、***、%(ip)s 等脱敏占位文本时，不得伪装成现场可执行字面量，也不得降级改写成 address、ip、error 等更宽泛关键词来绕过门禁；仍按原证据输出 Candidate、保留脱敏 pattern 并标 needs_review，由服务端归入 run_failed 交专家补现场值。match.pattern 数组中的每一项都必须能从逐字 evidence 或合法变量追溯，禁止在有证据项旁混入模型猜测项；regex pattern 必须能实际命中逐字 evidence，不能只表达意图却无法匹配自己的证据。keyword 不解释正则竖线；多个 QFK Matcher 关键字放在 match.pattern 数组，正则选择使用 regex。exists 只判断提取结果是否存在，不读取 pattern；需要命中具体内容时使用 keyword/regex/state。
   - 参数类型矩阵（最高优先级）：qkv_alert/qkv_task/qkv_dialog 的 acquire.args.keyword 必须是单个非空 string，对应一次 aCLI -k 查询；多个任务动作必须拆成多条 qkv Candidate，禁止写成 keyword 数组。只有 match.pattern 可以是 string 或 string[]；只有 extract.rows.include/exclude 使用 string[] 做候选行过滤。绝对禁止把这两类数组写入 qkv_* 的 acquire.args.keyword。
