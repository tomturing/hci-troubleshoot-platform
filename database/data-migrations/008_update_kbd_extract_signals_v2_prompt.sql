...
DECLARE
    has_rule11 BOOLEAN;
BEGIN
    SELECT content_template LIKE '%说明(description) 与 关键字 的边界%' INTO has_rule11
    FROM system_prompt
    WHERE name = 'kbd_extract_signals_v2';

    IF has_rule11 THEN
        RAISE NOTICE 'OK kbd_extract_signals_v2: 规则 11（说明/关键字边界）已落地';
    ELSE
        RAISE NOTICE 'WARN kbd_extract_signals_v2: 规则 11 未生效，请检查迁移是否执行';
    END IF;
END $$;
