...
CREATE TABLE IF NOT EXISTS verification_set (
    verification_set_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_verification_set_support_created
    ON verification_set (support_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS package_snapshot (
    ...
    CONSTRAINT package_snapshot_verification_fk
        FOREIGN KEY (verification_set_digest) REFERENCES verification_set(verification_set_digest) ON DELETE RESTRICT
);
...
DO $$ BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'verification_set_digest_format') THEN
        ALTER TABLE verification_set ADD CONSTRAINT verification_set_digest_format CHECK (
            verification_set_digest ~ '^sha256:[0-9a-f]{64}$'
        );
    END IF;
