...
def _activate_service(service_name: str) -> None:
    ...
    svc_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "backend", service_name))
    expect = os.path.normpath(os.path.join(svc_root, "app"))
    actual = os.path.normpath(getattr(sys.modules.get("app"), "__path__", [""])[0]) if "app" in sys.modules else ""
    if expect != actual:
