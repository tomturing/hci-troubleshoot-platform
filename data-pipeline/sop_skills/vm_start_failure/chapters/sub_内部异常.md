...
- #(680之前不管)acli --cluster platform nested status get acli --cluster system uname -a
```bash
#(680之前不管)
...
- acli system create_base_cpu_groupsacli system cgroup cpu init
```bash
...
- acli system ls -l /sf/data/local/dump acli system stat /sf/data/local/dump/core-kvm-xxxx
```bash
...
- acli vm config get  -v $vmid|grep os_installed
```bash
