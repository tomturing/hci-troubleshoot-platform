...
- **AI 空白气泡终极消灭**：修复 `conversations.py` 路由中由于 `asyncio.wait_for(..., timeout=0.1)` 导致的 AI 响应彻底被秒杀的问题。原设计在流式迭代中每隔 0.1s 轮询一次大模型响应，由于 TTFT（首字延迟）必然大于 0.1s，导致每次迭代都向大模型网络调用强行发送 Cancel 信号，从而彻底摧毁和终止了异步生成器，引发静默 DONE 产生空白气泡。重构为**非阻塞的并发 `merge_queue` 生产者-消费者队列模式**，首个 Token 在 0.1s 以上响应时依然流程完整接收。并在 `finally` 阶段加入了严密的协程取消与回收机制，100% 杜绝悬挂任务与内存泄露。
- **HTP 意图卡片确认无响应与高亮修复**：解决 HTP Agent S0 阶段由 TriageAgent 产生的候选意图确认气泡在点击时无反应的问题。对于 `intent_selection` 类型的卡片，前端绕过 ops-agent 独有的 `interactive-response` ACP 接口，直接映射为圆圈数字发送常规 message 触发下一轮状态机，并在请求体中透传 `metadata` 到后端；后端 conversations 路由及服务接收并在 PostgreSQL 中持久化该 metadata，实现了 100% 的高亮状态置灰高保真以及刷新历史后数据恢复的一致性。
