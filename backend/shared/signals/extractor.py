"""QFK 命令输出的确定性提取器（共享组件）。

该模块只处理已经执行完成的命令结果，不执行 shell、不解释 grep/awk，也不调用 LLM。
展示层可以继续使用截断输出；产出变量必须从完整物理流中提取。

禁止 import ``app.*`` 或 ``shared.observability.*``。
``get_complete_output()``（Redis 缓存恢复）保留在 agent-service 的兼容模块中，
因为它是执行基础设施关注点，不属于确定性文本提取。
"""
