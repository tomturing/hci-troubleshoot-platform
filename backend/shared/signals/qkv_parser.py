...
def parse_frontend_value(
    ...
    if query_type == "dialog":
        records = _extract_from_dialog_log(stdout_text)
        if not produces:
            return records
        projected = _extract_by_produces(records, produces)
        # line 是 Evidence 审计原文，不是业务变量；即使 produces 未声明也必须保留，
        # 否则动态字段映射会让审核/诊断结果失去可回查证据。
        for index, item in enumerate(projected):
            if index < len(records) and records[index].get("line"):
                item.setdefault("line", records[index]["line"])
        return projected

    # 2. 虚拟机控制台截图（vm_console）：输入不是命令 stdout，而是专用适配器
    #    回传的结构化视觉观察 JSON（VmConsoleObservation 固定 Schema）。
    if query_type == "vm_console":
...
def _extract_by_produces(items: list[Any], produces: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        for spec in produces:
            ...
            raw_candidates = path.split("|") if isinstance(path, str) else [path]
            candidates: list[str] = []
            for c in raw_candidates:
                if isinstance(c, str):
                    c_clean = c.strip()
                    candidates.extend([c_clean, c_clean.lower(), c_clean.upper()])
...
def first_complete_produced_record(
    records: list[dict[str, Any]], produces: list[dict[str, Any]] | None
) -> dict[str, Any] | None:
    """返回覆盖一条 producer 全部声明变量的首条记录。

    变量池按一条记录写入，不能把多条半成品记录拼接后宣称依赖已满足；否则缺失
    HOST 等关键值会把下游命令调度到错误目标。
    """
    names = [str(item.get("name") or "").strip().lower() for item in produces or [] if isinstance(item, dict)]
    ...
    return next(
        (
            ...
            if isinstance(record, dict) and all(record.get(name) not in (None, "") for name in names)
        ),
        None,
    )


def _extract_hardcoded(items: list[Any], query_type: str) -> list[dict[str, Any]]:
