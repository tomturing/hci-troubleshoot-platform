...
def _compile_tool_contract(tool: str, signal: dict[str, Any], checker: Any | None = None) -> str | None:
    """用保存契约和可选 tool_contract_checker 对同一条 acquisition 做编译校验。

    共享层只验证 acquire_args 模式。agent-service 可注入 checker 回调执行
    BackendSignal + build_acli_command 的在线编译校验；离线模式传 None 跳过。
    """
...
def compile_signal_plan(
    ...
    *,
    ...
    tool_contract_checker: Any | None = None,
) -> SignalPlan:
    ...
    for kbd in ordered:
        ...
        for index, signal in enumerate(kbd.signals, start=1):
            ...
            # qfk_log 会把 Matcher 关键字下推到采集命令；在 QFK 支持无 Matcher
