...
LANGFUSE_INGRESS = REPO_ROOT / "deploy" / "helm" / "hci-platform-obs" / "templates" / "ingress-langfuse.yaml"
...
def test_auth_paths_do_not_collide_with_langfuse() -> None:
    ...
    assert not collisions, (
        f"认证路径与 Langfuse Ingress 路径冲突，同一入口下 Traefik 会按最长前缀把请求截走：{collisions}"
    )
