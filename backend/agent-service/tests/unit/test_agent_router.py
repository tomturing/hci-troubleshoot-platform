"""
AgentRouter 单元测试（v4.0 架构）

测试路由逻辑：
  - ops-agent → OpsAgentAdapter（含降级）
  - pai-agent → PaiAgentAdapter（含降级）
  - htp-agent + S0          → TriageAgent
  - htp-agent + S1/S2/S3/S4 → InvestigationAgent
...
class TestAgentRouterRouting:
    ...
    @pytest.mark.asyncio
    async def test_route_all_investigation_stages(
        self, triage_agent, investigation_agent, ai_registry
    ):
        """S1/S2/S3/S4 均应路由到 InvestigationAgent"""
        router = AgentRouter(
            triage_agent=triage_agent,
            investigation_agent=investigation_agent,
            ai_registry=ai_registry,
        ...
        for stage in ("S1", "S2", "S3", "S4"):
            ...
            assert investigation_agent.call_count == 1, f"S{stage} 未路由到 InvestigationAgent"
...
class TestAgentRouterFallback:
    ...
    async def test_ops_agent_failure_fallback_to_investigation(
        ...
        """ops-agent 调用失败时应降级到 InvestigationAgent"""
    ...
    async def test_pai_disabled_fallback_to_investigation(
        ...
        """pai-agent 未启用时降级到 InvestigationAgent"""
        router = AgentRouter(
            triage_agent=triage_agent,
            investigation_agent=investigation_agent,
            ai_registry=ai_registry,
    ...
    async def test_pai_failure_fallback_to_investigation(
        ...
        """pai-agent 调用失败时降级到 InvestigationAgent"""
