...
def test_hallucination_detector_phantom_tool():
    detector = HallucinationDetector(tool_registry={"acli_vm_list": MagicMock(), "acli_service_restart": MagicMock()})

    # Mentioned VM list, but never executed
    text = "根据 acli_vm_list 的输出，我们发现虚拟机正常。"
    report = detector.detect(
        llm_text=text, executed_tools=["acli_service_restart"], tool_outputs=["Service restarted successfully"]
    )
...
def test_hallucination_detector_overconfident():
    ...
    report = detector.detect(llm_text=text, executed_tools=[], tool_outputs=[])
    assert report["has_hallucination"] is True
    assert len(report["overconfident_claims"]) > 0

    # Clean claim with uncertainty word
    text2 = "已确认是虚拟机网络配置文件损坏，可能是由于底层异常导致的。"
    report2 = detector.detect(llm_text=text2, executed_tools=[], tool_outputs=[])
...
def test_hallucination_detector_ungrounded_number():
    ...
    report = detector.detect(llm_text=text, executed_tools=[], tool_outputs=["Disk health is normal, usage is 50%"])
    assert report["has_hallucination"] is True
    assert "98.5%" in report["ungrounded_numbers"]

    # Grounded percentage
    text2 = "当前磁盘使用率为 50%"
    report2 = detector.detect(llm_text=text2, executed_tools=[], tool_outputs=["Disk health is normal, usage is 50%"])
    assert report2["has_hallucination"] is False
...
async def test_react_engine_schema_validation_success():
    ai_client = MagicMock()
    ai_client.invoke = AsyncMock(return_value=MagicMock(content='{"name": "vm-1", "status": "running"}', tool_calls=[]))
    ...
    engine = ReactEngine(ai_registry=ai_registry, tool_registry={}, tool_executor=MagicMock())

    events = []
    async for event in engine.execute(
        session_id="session-1", system_prompt="Test", messages=[], response_schema=SimpleSchema
    ):
        events.append(event)
...
async def test_react_engine_schema_validation_failure_blocks_write():
    ai_client = MagicMock()
    # Bad JSON content
    ai_client.invoke = AsyncMock(return_value=MagicMock(content='{"name": "vm-1", invalid_json}', tool_calls=[]))
    ...
    engine = ReactEngine(
        ...
        tool_registry={"acli_service_restart": MagicMock(risk_level=2)},
    ...
    tool_events = []
    async for event in engine._execute_tool_call(tool_call=tool_call_dict, session_id="session-1", step=1):
