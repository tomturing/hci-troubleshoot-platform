...
BACKEND_SIGNAL_JSON_SCHEMA = {
    ...
    "properties": {
        ...
        "signal_type": {
            "type": "string",
            "description": "信号类型描述（同 namespace，向后兼容）",
        },
        "target": {
            "type": "object",
        ...
        "sub_command": {
            "type": "string",
            "description": "（vm/network/storage/hardware/platform/system 专用）具体要拼接的 acli 子命名空间动作后缀",
        },
    },
}
...
3. 对于 vm/network/storage 等类型，必须将 sub_command 提取出来（如 "asan disk list"），QFK 会自动将其与 "acli storage" 拼接成完整命令；
