...
class TestTerminalHTTPAPI:
    ...
    def test_create_session_success(self, mock_terminal_service, mock_redis_manager):
        """测试创建会话成功"""
    ...
    def test_create_session_auth_failed(self, mock_terminal_service, mock_redis_manager):
        """测试创建会话认证失败"""
    ...
    def test_create_session_connection_refused(self, mock_terminal_service, mock_redis_manager):
        """测试创建会话连接被拒绝"""
    ...
    def test_close_session_success(self, mock_terminal_service, mock_redis_manager):
        """测试关闭会话成功"""
        app.state.terminal_service = mock_terminal_service
        app.state.redis_manager = mock_redis_manager
        ...
        response = client.post("/api/terminal/sessions/test-session-id/close")
    ...
    def test_close_session_not_found(self, mock_terminal_service, mock_redis_manager):
        """测试关闭不存在的会话"""
        ...
        response = client.post("/api/terminal/sessions/non-existent/close")

        assert response.status_code == 404

    def test_get_session_success(self, mock_terminal_service, mock_redis_manager):
        """测试获取会话信息成功"""
        app.state.terminal_service = mock_terminal_service
        app.state.redis_manager = mock_redis_manager
        ...
        response = client.get("/api/terminal/sessions/test-session-id")

        assert response.status_code == 200
        data = response.json()
        assert data["session_id"] == "test-session-id"

    def test_get_session_not_found(self, mock_terminal_service, mock_redis_manager):
        """测试获取不存在的会话"""
        ...
        response = client.get("/api/terminal/sessions/non-existent")
...
class TestTerminalWebSocketProtocol:
    ...
    def test_status_message_format(self):
        """测试 status 消息格式"""
        from app.models.terminal import TerminalStatusMessage, TerminalSessionStatus
