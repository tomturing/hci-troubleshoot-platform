"""
工具管理与 Prompt 管理路由 — API 网关代理层

负责将 /api/v1/tools 和 /api/v1/prompts 的请求代理转发到 conversation-service。
"""
...
router = APIRouter(prefix="/api/v1", tags=["tools-prompts-proxy"])
logger = get_logger("gateway-tools-prompts")
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            response = await client.request(
                method, url, json=payload, params=params, headers=req_headers
            )
            return response
