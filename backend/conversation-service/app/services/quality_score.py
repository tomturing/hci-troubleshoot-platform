...
class QualityScoreService:
    ...
    async def calculate_and_save(
        ...
        # 查询 conversation 获取消息数等信息
        result = await self.session.execute(
            select(Conversation).where(Conversation.case_id == case_id).order_by(Conversation.started_at.desc())
        )
        conv = result.scalar_one_or_none()

        if conv:
            message_count = conv.message_count or 0
            repeat_question_count = getattr(conv, "repeat_question_count", 0) or 0
            # 计算会话时长（如果有结束时间）
            if conv.ended_at:
                session_duration = int((conv.ended_at - conv.started_at).total_seconds())
            else:
                session_duration = int((datetime.now(UTC) - conv.started_at).total_seconds())
        else:
            message_count = 0
