...
class TestConversationServiceIntegration:
    ...
    async def test_health_check(self):
        ...
        async with (
            ...
            httpx.AsyncClient(transport=ASGITransport(app=app), base_url=self.BASE_URL) as client,
        ):
    ...
    async def test_create_and_get_conversation(self):
        ...
        async with (
            ...
            get_resp = await client.get(f"{self.BASE_URL}/api/conversations/{conv_id}")
    ...
    async def test_get_nonexistent_conversation(self):
        ...
        async with (
            ...
            response = await client.get(f"{self.BASE_URL}/api/conversations/{fake_id}")
    ...
    async def test_send_message_and_stream(self):
        ...
        async with app.router.lifespan_context(app):
            ...
            # 2. 发送消息
            with patch("app.services.ai_client.OpenClawAssistant.chat_completion_stream", side_effect=mock_stream):
                async with httpx.AsyncClient(transport=ASGITransport(app=app), base_url=self.BASE_URL) as client:
                    response = await client.post(
                        f"{self.BASE_URL}/api/conversations/{conv_id}/message",
                        json={"role": "user", "content": "你好，请帮我排查问题", "case_id": "Q202602220003"},
                        headers={"X-Trace-ID": "inttest-conv-003"},
                    )
    ...
    async def test_get_conversation_messages(self):
        ...
        async with app.router.lifespan_context(app):
            ...
            with patch("app.services.ai_client.OpenClawAssistant.chat_completion_stream", side_effect=mock_stream):
                async with httpx.AsyncClient(transport=ASGITransport(app=app), base_url=self.BASE_URL) as client:
                    ...
                    async with client.stream(
                        "POST",
                        f"{self.BASE_URL}/api/conversations/{conv_id}/message",
                        json={"role": "user", "content": "问题描述", "case_id": "Q202602220004"},
                        headers={"X-Trace-ID": "inttest-conv-004"},
                    ) as response:
                        assert response.status_code == 200
                        # 消费流内容以确保后端迭代完成
            ...
            # 3. 查询消息历史
            async with httpx.AsyncClient(transport=ASGITransport(app=app), base_url=self.BASE_URL) as client:
                response = await client.get(f"{self.BASE_URL}/api/conversations/{conv_id}/messages")
                assert response.status_code == 200
