...
def test_resolve_env_variable_hci_version():
    ...
    env_context = EnvironmentContextResponse(
        env_info={"hci_version": "6.8.0", "cluster_name": "prod-cluster"},
        alert_logs=[],
        task_logs=[]
    )
    # 大小写和下划线不敏感匹配
        ...
        alert_logs=[
            {
                "host": "10.0.1.10",
                "target": "node-1",
                "description": "disk failed on host 10.0.1.10"
            },
            {
                "host": "192.168.1.1",
                "target": "node-2",
        ...
        alert_logs=[
            {
                "host": "10.0.1.10",
                "target": "node-1",
                "description": "磁盘寿命异常 [SN: 500605B001234567] 告警"
            }
        ],
        task_logs=[]
    )
