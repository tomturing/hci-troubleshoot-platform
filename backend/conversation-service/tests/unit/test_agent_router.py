...
覆盖三个核心场景（Issue 8）：
1. AgentRouter 注入时正确产出 AgentTextChunk
2. ops-agent 不可达触发 AgentUnavailableError 后能降级到 htp 并产出 fallback notice
3. AgentStageUpdate 仅在 stage 符合 S\\d 时透传给 ConversationService 的调用者
"""
...
from app.core.agent_port import AgentEvent, AgentStageUpdate, AgentTextChunk, AgentUnavailableError

# ── 辅助工厂 ──────────────────────────────────────────────────────────────────

def _make_mock_adapter(events: list[AgentEvent]):
    """创建产出固定事件序列的 mock 适配器。"""

    async def _process(**_kwargs: Any) -> AsyncGenerator[AgentEvent, None]:
        for e in events:
            yield e
...
def _make_failing_adapter(reason: str = "connection refused"):
    """创建抛出 AgentUnavailableError 的 mock 适配器。"""

    async def _process(**_kwargs: Any) -> AsyncGenerator[AgentEvent, None]:
        raise AgentUnavailableError(agent_name="ops-agent", reason=reason)
...
async def _collect(router: AgentRouter, **kwargs: Any) -> list[AgentEvent]:
    """收集 AgentRouter.process() 的所有输出事件。"""
...
class TestAgentRouterTextChunk:
    """场景 1：注入 AgentRouter 时能正确产出 AgentTextChunk。"""

    @pytest.mark.asyncio
    async def test_htp_path_yields_text_chunks(self):
        """assistant_type 非 ops-agent 时走 htp 路径，正确产出文本。"""
        chunks = [AgentTextChunk(content="hello"), AgentTextChunk(content=" world")]
        htp = _make_mock_adapter(chunks)
        router = AgentRouter(htp_adapter=htp)
        ...
        assert len(events) == 2
        assert all(isinstance(e, AgentTextChunk) for e in events)
    ...
    async def test_ops_agent_path_yields_text_chunks(self):
        ...
        chunks = [AgentTextChunk(content="ops reply")]
        ops = _make_mock_adapter(chunks)
        htp = _make_mock_adapter([])
        router = AgentRouter(htp_adapter=htp, ops_agent_adapter=ops)
...
class TestAgentRouterFallback:
    ...
    async def test_fallback_on_brain_unavailable(self):
        """ops-agent 抛 AgentUnavailableError 后，降级到 htp 并发送降级提示。"""
        ops = _make_failing_adapter(reason="connect error")
        htp_chunks = [AgentTextChunk(content="htp fallback")]
        ...
        text_contents = [e.content for e in events if isinstance(e, AgentTextChunk)]
    ...
    async def test_no_ops_adapter_uses_htp(self):
        ...
        htp_chunks = [AgentTextChunk(content="htp only")]
        htp = _make_mock_adapter(htp_chunks)
        router = AgentRouter(htp_adapter=htp, ops_agent_adapter=None)
...
class TestAgentRouterStageUpdate:
    """场景 3：AgentStageUpdate 仅在 stage 符合 S\\d 格式时透传。"""

    @pytest.mark.asyncio
    async def test_s_stage_forwarded(self):
        """S0-S6 格式 stage 应正常产出。"""
        events = [AgentStageUpdate(stage="S3"), AgentTextChunk(content="text")]
        ...
        stage_events = [e for e in result if isinstance(e, AgentStageUpdate)]
        assert len(stage_events) == 1
        assert stage_events[0].stage == "S3"

    @pytest.mark.asyncio
    async def test_ops_internal_stage_forwarded_from_router(self):
        """AgentRouter 透传 ops-agent 的内部 stage（过滤由 ConversationService 处理）。"""
        events = [AgentStageUpdate(stage="intake"), AgentTextChunk(content="done")]
        ...
        # AgentRouter 本身不过滤 stage，过滤在 ConversationService 层
        stage_events = [e for e in result if isinstance(e, AgentStageUpdate)]
        assert len(stage_events) == 1
