...
class TestHelperFunctions:
    ...
    def test_timeout_score(self):
        """测试超时评分修正"""
        assert _timeout_score(4000) == 70  # > 1 hour
        assert _timeout_score(2700) == 55  # 30-60 min
        assert _timeout_score(1801) == 55  # > 30 min
        assert _timeout_score(1800) == 40  # = 30 min（边界值）
        assert _timeout_score(1200) == 40  # < 30 min
