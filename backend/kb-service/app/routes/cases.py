"""
KB Service — KBD Cases 检索路由（供 agent-service 使用）
...
logger = get_logger("kb-service-cases")
router = APIRouter(prefix="/api/kb", tags=["cases"])
...
async def _entry_to_case_with_revision(
    ...
    *,
    ...
    await DynamicResourceLoader(session).audit_usage(
        snapshot,
        UsageRecord(
            consumer="kb-service.cases_search",
            status="matched",
            trace_id=trace_id,
...
async def search_cases_with_steps(
    ...
    """KBD Cases 语义检索（agent-service 专用）
    ...
    try:
        ...
        logger.warning(
            event="cases_search_embedding_failed",
            error=str(e),
