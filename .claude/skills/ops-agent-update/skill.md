...
> **用途**：将 ops-agent 仓库 `feature-hci` 分支的最新 commit 同步到 hci-platform-env 的 values.yaml，触发 ArgoCD 自动部署。

## 背景

- **ops-agent 仓库**：`https://github.com/P3n9W31/ops-agent`，独立迭代
- **使用分支**：`feature-hci`
- **镜像仓库**：`ghcr.io/p3n9w31/ops-agent:latest`（或具体 tag）
- **环境仓库**：`hci-platform-env`，存放各环境的 `opsAgent.image.tag`

ops-agent 不在本项目 CI 构建范围内，需要手动同步镜像 tag 到环境仓库。
