...
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                          │
│  每个OpenClaw Pod / Gateway:                                                │
│  • OpenAI 兼容 /v1/chat/completions 接口                                  │
│  • 通过 ZAI_API_KEY 对接上游 z.ai GLM 模型                                 │
│  • 支持 stream=true 流式 SSE 响应                                       │
│  Port: 18789 (容器内)                                                  │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
...
│  │ • trace_id      │  │ • Log Aggregator│  │ • Visualization         │ │
│  │ • Stdout JSON   │  │                 │  │                         │ │
│  └─────────────────┘  └─────────────────┘  └─────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────┘
```
...
3. 服务层处理
   Conversation Service (trace_id: current span trace_id)
...
→ 本地开发以 Docker 容器运行，生产可部署为 K8s Pod
→ OpenAI 兼容 HTTP API (Port 18789)
→ 通过 ZAI_API_KEY 调用上游 z.ai GLM 模型
```

**关键配置**:
```yaml
OpenClaw Container:
  Image: 本地构建 (node:22-bookworm)
  User: node (非 root，安全加固)
  Environment:
    - ZAI_API_KEY          # 上游模型密钥
    - OPENCLAW_GATEWAY_TOKEN  # 网关鉴权 Token
    - HOME=/home/node
  Volumes:
    - ~/.openclaw:/home/node/.openclaw   # 配置与工作区
  Health Check:
    # 真实 OpenClaw 无 /health 端点，改用 POST /v1/chat/completions 空 payload
    # 400/422 视为可达；401/403/404/405 视为不健康
    Method: POST /v1/chat/completions (empty JSON body)
    Expected: 400 or 422 = healthy
```
...
services:
  ...
  - case-service         # 工单服务
  - conversation-service # 对话服务，调用 OpenClaw
