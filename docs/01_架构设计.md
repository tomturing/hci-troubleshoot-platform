...
- **创建日期**: 2026-02-15
- **更新日期**: 2026-02-27
- **状态**: 架构优化阶段 — 从 OpenClaw 单助手演进到 AI Assistant 多助手池化架构
...
| 维度 | v1.0 (单助手) | v2.0 (多助手池化) |
|------|--------------|------------------|
| AI层 | OpenClaw Pod Pool (单一类型) | AI Assistant Pod Pool (多类型独立池) |
| 调度 | allocate_pod(case_id) | allocate_pod(case_id, assistant_type) |
| 对话 | 硬编码 OpenClawClient | AIAssistantClient 抽象 (策略模式) |
| 知识库 | 无 | Knowledge Base Service (RAG 三层注入) |
| 数据模型 | conversation.openclaw_pod_id | conversation.assistant_type + pod_id |
| 助手选择 | 无 (固定 OpenClaw) | 用户可选 / 生产环境系统自动分配 |
| 评估 | 无 | assistant_evaluation 表 (准确率对比) |
| 扩展 | 修改代码 | AssistantRegistry 配置化注册，零代码接入 |
...
5. **多助手**: AssistantRegistry 设计完成，当前仅 OpenClaw 已实际接入

### 10.2 v2.0 → v2.1 演进计划

| 优先级 | 任务 | 说明 |
|--------|------|------|
| P0 | Conversation Service AIAssistantClient 抽象 | 替换硬编码 OpenClawClient |
| P0 | Scheduler Service PodPoolManager 重构 | 支持多类型独立池 |
| P0 | DB Schema 扩展 | 添加 assistant_type 字段到 case、conversation 表 |
| P1 | KB Service 微服务骨架 | RAG pipeline 基础实现 |
| P1 | Customer UI 助手选择器 | 工单创建时选择 AI 助手 |
...
*文档版本: 2.0 | 创建: 2026-02-15 | 更新: 2026-02-27*
