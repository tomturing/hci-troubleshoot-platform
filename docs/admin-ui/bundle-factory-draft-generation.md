...
因此，Draft 的可重复性由三项冻结事实共同保证：完整 Signal 路由/Intent 集合、C1 返回的 KBD/Signal/Tool/Policy digest，以及统一变量池渲染后的 Manifest digest。生成算法变化时必须递增 `compiler_revision`（当前为 `bundle-factory-v2`），让 Registry 为同一 KBD 派生新的 Draft，而不是复用旧算法产物。
