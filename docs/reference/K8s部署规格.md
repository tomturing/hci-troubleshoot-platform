...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| 1.5 | 2026-03-30 | hci-platform-data-staging 使用多源配置引用环境仓库 values（PR #75） |
| 1.4 | 2026-03-30 | 修复 prometheus Service YAML 分隔符缺失问题（PR #73） |
| 1.3 | 2026-03-30 | 增加 db-password-check PreSync Job 超时到 180s（PR #72） |
| 1.2 | 2026-03-29 | 修复 Prometheus 告警规则 namespace 过滤条件（PR #70） |
| 1.1 | 2026-03-29 | 补充 nginx emptyDir volumes 配置说明（PR #68 修复） |
| 1.0 | 2026-02-15 | 初版设计文档 |
...
可观测性栈按部署域拆分为独立 Application 并与业务服务解耦：

- local dev：`hci-platform-obs-dev`（`deploy/gitops/argo-apps/local/hci-platform-obs-dev.yaml`）
- cloud staging：`hci-platform-obs-staging`（`deploy/gitops/argo-apps/cloud/hci-platform-obs-staging.yaml`）
- cloud prod：`hci-platform-obs-prod`（`deploy/gitops/argo-apps/cloud/hci-platform-obs-prod.yaml`，server 指向 `192.168.0.3:6443`）
...
sources:
  - repoURL: hci-troubleshoot-platform  # Helm Chart
    helm:
      valueFiles: [$values/environments/staging/values.yaml]  # staging 环境配置
