...
可观测性栈由独立 ArgoCD Application `hci-platform-obs` 管理（`deploy/gitops/argo-apps/hci-platform-obs.yaml`），与业务服务 Application 解耦。prod 集群使用对应的 `hci-platform-obs-prod`（部署到 `hci-observability` 命名空间，server 指向 `192.168.0.3:6443`）。
