...
### 7.4.1 Scope 环境字段与拓扑标签约定

`scope` 不是完整的资产或网络拓扑快照，而是案例裁决前使用的一组小而封闭的适用性标签。KBD 声明期使用 `products`、`versions`、`components`、`topology_constraints`；运行期以同一语义的顶层环境字段 `product`、`version`、`components`、`topology` 进行比较。

| KBD Scope 声明 | 运行期环境字段 | 含义 | 示例 |
|---|---|---|---|
| `products` | `product` | 产品/平台身份 | `HCI` |
| `versions` | `version` | 产品版本 | `6.11.1_R1` |
| `components` | `components` | 已确认的故障功能域 | `虚拟机`、`存储`、`迁移` |
| `topology_constraints` | `topology` | 会影响案例适用性的部署结构标签 | `external_storage`、`dual_site` |

其中 `topology` **不是**节点清单、网络连接图或全量集群配置。它只是稳定、可枚举的部署结构标签集合；只有部署结构会改变案例成立条件、采集路径或处置安全性时，KBD 才应声明对应约束。

例如，只适用于外置存储虚拟机迁移的案例可声明：

```json
{
  "products": ["HCI"],
  "components": ["虚拟机", "迁移"],
  "topology_constraints": ["external_storage"]
}
```
...
```json
{
  "product": "HCI",
  "components": ["虚拟机", "迁移"],
  "topology": ["external_storage", "dual_site"]
}
```

判定采用包含关系：KBD 声明的每个 `topology_constraints` 都必须出现在 `environment.topology` 中。若 KBD 要求 `external_storage`、现场只提供空数组，则案例为 `NOT_APPLICABLE`；若现场根本没有 `topology` 字段，则事实不足，为 `UNKNOWN`，最终保持 `INCONCLUSIVE`。空的 `topology_constraints` 表示该 KBD 不限制部署拓扑，不要求现场提供 `topology`。

常用标签的语义如下，新增标签必须先进入受控目录和对应采集/验证方案，不能在 KBD 正文或 Prompt 中自由编造：

| 标签 | 表示的适用环境 |
|---|---|
| `external_storage` | 外置存储场景 |
| `physical_gpu` | 物理 GPU 直通或 vGPU 场景 |
| `dual_site` | 双站点、双活或容灾部署 |
| `cross_cluster` | 跨集群迁移、复制或通信 |
| `stretched_cluster` | 跨机房拉伸集群 |
| `witness` | 存在见证节点的仲裁拓扑 |
| `nested_virtualization` | 嵌套虚拟化环境 |

运行期归一化必须可追溯，不能用自由文本猜测 Scope：`env_info.hci_version` 是受控集群采集字段，可归一为 `product=HCI` 与 `version`；`components` 只可补入已经由 KB 目录校验、且由用户在 S0 确认的分类一级域；`topology` 只透传上游已有的结构化标签。任何没有权威来源的字段保持缺失，由 Scope Gate fail-closed，而不是为了执行 KBD 伪造“适用”。
...
当前默认模型为 `kimi-k2.5`，默认 `VISION_MAX_TOKENS=8192`。密集日志仍可能超过模型上下文窗口，服务端必须保留 `finish_reason=length` 门禁，不能把截断结果写入知识库；真实图片 Golden Regression 仍需持续补充。
...
8. Scope 在真实调度和 Replay 前统一执行；产品、版本、组件或拓扑缺失时保持 inconclusive，不会因后续信号满足而被覆盖为 confirmed。conversation-service 在转发环境时将受控的 `env_info.hci_version` 归一为顶层 `product=HCI`、`version`，并仅将已由 KB 目录校验且用户确认的 S0 分类一级域补入 `components`；没有这些权威来源时不得猜测，继续 fail closed。QFK 在执行前使用现场 `env_context + variable_pool` 渲染 `{{HOST}}/{{VM}}/{{PID}}/...`；Compiler 的代表值只用于 shape 检查，不能替代现场变量。
...
| Vision 默认 max tokens 8192 | `backend/kb-service/app/services/vision_processor.py:88` |
