...
Conclusion Gate 没有错误；它正确阻止了在 30880 尚未排除时输出唯一根因。真实缺口是 **`kbd_differential.py` 的 PARTIAL 报告路径丢弃了已支持候选**：调用报告模板时把 `supported` 传成空列表，随后又把完整候选集都标为“未确认”。Bundle 的 RouteKey 和 `fixture_not_found` 行为不需要修改。
...
原工单在恢复全候选后得到 `PARTIAL` 是正确门禁结果；修复重点是报告必须明确展示已命中的参考案例及现场证据，同时继续隐藏根因/解决方案并禁止 S4。不得通过放宽 Conclusion Gate、缩窄候选或修改 Bundle 来掩盖未决候选。
