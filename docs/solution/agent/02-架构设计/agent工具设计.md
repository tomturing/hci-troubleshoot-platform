...
| 2026-07-16 | v3.0 | **工具命名规范统一与工具集精简**：① 工具命名强制 `snake_case`，正则 `^[a-z][a-z0-9_]{0,63}$`，在后端 `validate_tool_payload()`（`TOOL_NAME_INVALID_FORMAT`）、DB `chk_tool_definition_tool_name_format` CHECK 约束、前端 `ToolManageView.vue` 表单三处一致生效；② 11 个点号工具（qkv.alert 等）重命名为下划线（qkv_alert 等），同步更新 acquirer 路由/解析（`kbd_differential.py` 的 `startswith("qkv_")`/`split("_")`）与 `ACQUIRER_CATALOG`、system_prompt 模板示例；③ 删除 6 个低频工具（acli_plugin_asys / acli_plugin_netdoctor / acli_plugin_vm_start / acli_plugin_vm_suspend / get_cluster_detail / get_vm_list）及其种子与 dynamic_resource 快照。详见 [工具命名规范统一与工具集精简决策.md](工具命名规范统一与工具集精简决策.md) |
