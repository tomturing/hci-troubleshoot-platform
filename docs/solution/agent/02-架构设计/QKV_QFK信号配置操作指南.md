...
QKV 的 `acquirer_args.keyword` 是 `acli <task|dialog|alert> get -k` 的**检索词**，不是自由描述。它必须能命中平台真实记录命名，即与本案例所属**分类基线**标签语义对齐，否则查不到记录、信号恒为假（静默漏诊）。
...
| v1.2 | 2026-07-23 | 新增 §2.3 关键字须对齐分类基线（语义约束）、§2.4 校验与容忍原则（输入容忍 + 实时基线 + 只软校验输出）；§5.2 补"分类基线不对齐"常见错误。设计详见 `solution/events/2026-07-23-QKV关键信号关键字分类基线校验设计.md` |
