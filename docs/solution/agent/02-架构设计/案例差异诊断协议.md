...
> **当前实现校正（2026-08-20）**：本文早期的 `top-K`、`_pick_best_step()`、按工具覆盖频率
> 选步和“剩余 1～2 篇即停止”仅代表历史草案，不是当前 KBD 运行时行为。当前实现以
> [KBD 主动诊断信号调度与证据闭环算法设计](../../knowledge-base/KBD主动诊断信号设计.md)
> 和本轮事件文档为准；下文历史算法不应作为实现依据。
