...
> **状态：P0（契约层）+ P1（在线执行与 S6 闭环）+ P2（离线历史回放）已实现（2026-09-10）。执行层默认
...
>   `extract_requirements` 跳过 qkv_effect；验包后只读回放同一 `diagnosis_run` 的已完成效果记录；
...
> （告警未消除、状态未翻转、性能未恢复、操作未生效）。这类现象不是事件，而是
> “期望状态 × 实际观测”的关系判定，现有所有事件流中结构性缺席。
>
> 本文是对 [QKV/QFK 信号模型 v2 参考](../knowledge-base/关键信号架构设计.md)、
...
本信号把“工具成功 ≠ 信号通过”延伸为 **“动作成功 ≠ 效果达成”**，覆盖操作后效果验证与
症状确认两个场景。
...
| `qfk_storage` 存储快照 | 已批准的只读存储查询输出 | 迁移目标对象的文件/容量/状态应持续变化或达到目标状态 |
...
**不新增自由文本判定**。若现有 8 类确实无法表达某类期望，走正式提案扩展 matcher 集合并
补充测试，不允许在 KBD 层绕过。
...
启用 `expectation.progress.mode=change_required` 时，每次有效观测同时保存输出 SHA-256 指纹。
首个指纹为 `baseline`，后续变化为 `in_progress`，连续未变化超过停滞窗口为 `stalled`；客户结果卡
会展示“仍在推进”或“已停滞”，使窗口结束的 `inconclusive` 与实际未达预期可区分。
...
| `expectation.matcher` | object | 是 | 8 类封闭 matcher 之一；`expected` 翻转取反。 |
...
| `expectation.progress` | object | 否 | `change_required` 时比较受控观测输出的 SHA-256 快照；首次为 baseline，变化为 in_progress，连续无变化超过 `idle_after_seconds` 为 stalled。至少需要一次复核。 |
| `usage` | enum | 否 | `remediation_verify`（默认，操作后效果验证）/ `symptom_confirm`（S1 症状确认）。 |
...
>
> 进展观察不开放目录、路径或 Shell 参数。它比较已批准的告警、任务、弹框、控制台或 `qfk_storage` 存储查询输出；后者继续使用既有 aCLI 参数校验和资源选择器。迁移目录级文件快照仍需要独立受控观测原语。
...
3. `expectation.observation.tool` 必须属于封闭观测通道集合，且 `args` 原样通过对应原语的
   `acquirer_args` 校验；`matcher` 必须属于 8 类封闭集合；任一自由文本判定字段直接 422；
...
| matcher 封闭性 | 8 类之外的 matcher 类型 422；观测通道之外的 tool 422；观测 args 按原语契约校验。 |
...
| 判定 | 8 类 matcher × `expected` 翻转回放；三态合成表全覆盖；词表版本化回放。 |
