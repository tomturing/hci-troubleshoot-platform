...
为了在后续的迭代中，以不破坏当前稳定运行为前提，平滑向该架构过渡，我们建议分为三步走：

1. **第一步（解耦 execution 动作）**：
   在 `memory/variable_pool.py` 中，**禁止**直接编写任何底层的 HTTP 客户端调用或系统命令组装代码。所有自动获取逻辑（`tool_call` 的具体实现）必须作为参数从外部传入（即利用好已预留的 `tool_executor` 传入参数），实现 `variable_pool.py` 的物理减肥。
2. **第二步（物理目录重组）**：
   在 `backend/agent-service/app/` 下创建 `memory/variable_pool/` 子文件夹。将大文件 `variable_pool.py` 拆解为：
   - `pool.py`（纯粹的工作记忆 CRUD 操作）
   - `engine.py`（JIT 策略解析与路由控制）
3. **第三步（规范化 JIT 交互插槽）**：
   将 S0 的分类卡片规范整合进 `variable_pool` 的 Schema 中。将故障分类定义为一个标准的 `user_confirm` 变量（变量名 `target_sop_category`），使其与 SOP 执行过程中的普通变量（如 `vm_name`）共享完全相同的后端调度、中断、挂起、落库与前端元数据渲染引擎，实现**全智能体生命周期的交互模型统一**。
