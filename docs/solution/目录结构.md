...
> ⚠️ **注意与架构蓝图的映射说明**：
> 上述结构为**当前物理落地的实际目录结构**。在我们的微内核架构设计演进路线中，定义了**未来完全解耦的理想目标蓝图（Target Architecture Blueprint）**：
> 1. 将 `tools/acli/` 和 `tools/sop/` 进一步按语义归纳为 `tools/system/`（主动命令类）和 `tools/interactive/`（人机交互类），将 `base_tool.py` 更名为 `base.py`。
> 2. 将 `adapters/clients/scp_client.py` 承载的中断挂起职责，物理分拆为 `adapters/interaction/interrupt.py`（人机交互中断层）。
> 3. 将 `adapters/agents/htp/` 内置的正则和自然语言抽取逻辑，物理分拆为 `adapters/agents/htp/parsers.py`。
> 
> 目前，我们在 `memory/variable_pool` 中率先落地了这套微内核架构，并通过无状态注入外部客户端，完美支持了现有的物理拓扑，实现了 100% 的渐进式演进。
