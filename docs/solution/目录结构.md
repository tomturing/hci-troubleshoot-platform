...
`agent-service` 是整个 HCI 平台最复杂的 AI 推理核心，负责处理 ReAct 循环、事件分发以及工作记忆管理。其目录结构如下：
...
│   └── clients/                # 外部服务客户端（SCP 客户端、acli 交互客户端）
│
├── memory/                     ← 【控制核：工作记忆控制面】
│   └── variable_pool/          # JIT 变量获取引擎（微内核设计）
│       ├── __init__.py         # 门面层（Facade）：提供 100% 兼容的 API 导出
│       ├── pool.py             # 存储与状态核心（State）：定义 VariableRequestResult 与内存快照表达
│       └── engine.py           # 策略决策内核（Control）：执行缓存扫描、依赖检查与 JIT 路由调度
│
└── tools/                      ← 【数据面：无状态执行器】
    ├── base.py                 # 工具注册器基类
    ├── system/                 # 系统级主动触达工具（如 node_ip 获取、K8s 容器检查）
    └── interactive/            # 交互类候选生成器（如生成 S0 故障选项列表）
```
