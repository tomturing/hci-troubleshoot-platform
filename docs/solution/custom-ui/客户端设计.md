...
| v1.6 | 2026-04-29 | Bug 修复：`completeCaseCreationFlow()` 每次无条件调用 `caseApi.getById()` 导致重复请求；改为仅在 `currentCase` 不存在或 case_id 不匹配时才拉取。catch 块新增 `console.error` 和结构化错误信息（`e.response?.data?.detail \|\| e.message`）；若 `currentCase` 已有旧值则降级继续流程，不中断对话。影响文件：`frontend/customer/src/stores/chat.ts` |
