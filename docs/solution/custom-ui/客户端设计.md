...
| v1.9 | 2026-05-09 | **助手选择恢复修复（刷新场景）**：`chat.ts` 在 `initializeChatState()` 检测到活跃工单后立即调用 `_restoreAssistantFromCase(activeCase)`，不再依赖用户点击“续上”后才恢复；`CaseCreateDialog.vue` 在弹窗每次打开时同步 `caseForm.assistantType = chatStore.selectedAssistant`，避免沿用旧挂载状态导致助手显示错乱；`chat.assistant.spec.ts` 补充对应单测覆盖。 |
