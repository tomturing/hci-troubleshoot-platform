...
| 日期 | 版本 | 变更内容 | 关联事件文档 |
|------|------|---------|------------|
| 2026-04-17 | v0.3 | AI 助手选择器智能显示：删除前端环境变量硬开关，改为后端 API 响应驱动（`show_selector` 字段）；新增 `capabilities` 能力标签显示；关联配置详见 [AI助手设计.md §10](../ai-assistant/AI助手设计.md) | 本 PR |
...
```
GET /api/assistants → {
  assistants: [{type, display_name, capabilities, available, is_default}],
  show_selector: boolean,
  default_assistant: string
}

前端状态：
  showAssistantSelector ← response.show_selector
  selectedAssistant ← response.default_assistant 或第一个可用助手
```
