...
| v1.13 | 2026-05-10 | **修复：自由文本提交后选项未置灰（4+1模式）**：`InteractiveOptions.vue` 新增 `forceDisabled` prop；禁用条件改为 `selectedOptionId !== null \|\| forceDisabled`；`MessageBubble.vue` 传入 `:force-disabled="interactiveSubmitted"`，覆盖选项提交和自由文本提交两种场景 |