...
- **版本**: 0.13（SSH 集成体验重构 — 统一入口 + upsert 机制）
- **创建日期**: 2026-03-11
- **状态**: 开发中
- **关联文档**: [../ai-assistant/AI助手设计.md §10](../ai-assistant/AI助手设计.md)、[../接口设计.md](../接口设计.md)、[events/2026-04-21-SSH集成创建工单方案.md](events/2026-04-21-SSH集成创建工单方案.md)
...
## 5. SSH 集成体验架构（v0.13 重构）

### 5.1 组件架构

v0.13 将原来散落在 `CaseCreateDialog`、`TerminalPanel` 中的 SSH 表单和连接逻辑统一收敛到两个新组件：

```
App.vue
└── SshConnectDialog.vue           # 弹框容器（薄层）
    └── SshFlowPanel.vue           # SSH 连接全流程状态机
        ├── mode: 'create-case'    # 创建工单入口
        └── mode: 'terminal-only'  # 终端侧边栏入口

CaseCreateDialog.vue               # 精简版（195行）：仅保留基础表单 + 两个操作按钮
TerminalPanel.vue                  # 移除 SSH 登录表单，改为"连接 SSH"按钮
```

### 5.2 SshFlowPanel 状态机

```
checking-bridge
    │
    ├─ Bridge 未运行 ──→ bridge-not-running（显示下载说明 + 刷新按钮）
    │
    └─ Bridge 就绪 ──→ ready（SSH 配置表单，自动填充 localStorage）
                           │
                           └─ 点击连接 ──→ connecting
                                              │
                                              ├─ 失败 ──→ error（显示错误 + 重试）
                                              ...
                                              └─ 成功 ──→ acli-check
                                                              ...
                                                              └─ acli 存在 ──→ collecting
                                                                                   ...
                                                                                   └─ 完成 ──→ done
                                                                                                │
                                                                        create-case 模式 ──→ completeCaseCreationFlow()
                                                                        terminal-only 模式 ──→ openTerminalSidebar()
```

### 5.3 两种入口对比

| 入口 | 触发方式 | SshFlowPanel mode | 完成后动作 |
|------|---------|-------------------|-----------|
| 创建工单时 | CaseCreateDialog → "连接 SSH 并创建工单" | `create-case` | `completeCaseCreationFlow()` 创建工单 + 启动对话 |
| 终端侧边栏 | TerminalPanel → "连接 SSH" / `checkAndOpenTerminal()` | `terminal-only` | `openTerminalSidebar()` 打开终端 |

### 5.4 `checkAndOpenTerminal()` 三态决策
...
function checkAndOpenTerminal() {
    if (sshStatus === 'connected') {
        ...
        // 有工单但未连接 → 打开 SSH 弹框（terminal-only 模式）
        openSshFlowDialog('terminal-only', currentCase.id)
    }
}
```
