...
- **版本**: 0.39（SSH 连接后 acli 自动检测与静默安装更新）
- **创建日期**: 2026-03-11
- **状态**: 开发中
- **关联文档**: [../agent/AI助手设计.md §10](../agent/AI助手设计.md)、[../接口设计.md](../接口设计.md)、[events/2026-04-23-SSH-UX全场景重构方案.md](events/2026-04-23-SSH-UX全场景重构方案.md)、[events/2026-09-20-acli自动检测与安装更新方案.md](events/2026-09-20-acli自动检测与安装更新方案.md)
...
| 日期 | 版本 | 变更内容 | 关联事件文档 |
|------|------|---------|------------|
| 2026-09-20 | v0.39 | **SSH 连接后 acli 自动检测与静默安装更新**：① `terminal_bridge` 增加 `acli_sync` 消息处理，实现 `uname -m` 架构识别、`acli --version` 探测、深信服源 `Range: bytes=0-0` 秒级元数据预检、`/sf/data/local/` 磁盘门禁校验与 `--install --force` 静默多节点部署；② 前端新增 `acliManager.ts` 并在 `CaseCreateDialog.vue` 与 `chatStore` 的 `ssh_connected` 触发点自动调度，确保环境数据采集前 `acli` 100% 可用 | [2026-09-20-acli自动检测与安装更新方案.md](events/2026-09-20-acli自动检测与安装更新方案.md) |
| 2026-07-29 | v0.38 | **Markdown 命令执行信任边界收敛**：普通 Assistant Markdown 代码块只允许复制或用户显式“发送到终端”，不再读取 Aggressive 偏好、不倒计时、不通过 `ssh_input` 自动执行，也不把结果伪装成 `role=user` 回灌；自动执行只接受后端签发且携带 `exec_id` 的 `agent_exec_command` 结构化事件 | [2026-07-29 Terminal Bridge 真实入口 P0 修复与端到端重验](../../verify/events/2026-07-29-terminal-bridge真实入口P0修复与端到端重验.md) |
| 2026-06-08 | v0.37 | **ReAct 高危工具确认弹窗事务对齐（PR #417 / T1-2）**：`chat.ts` `pendingConfirm` 类型扩展 `exec_id`、`input_hash` 字段，接收 SSE `confirm_request` 事件时同步捕获；`handleConfirmResult` 旧实现请求不存在的 `/api/conversations/{id}/confirm` 路由且未回传事务标识，改为命中标准 `/interactive-response`（`kind=tool_confirm`），body 携带 `request_id=exec_id` 与 `outcome.input_hash` / `metadata.input_hash`，与 `MessageBubble.vue` 工具卡片确认路径完全对齐，受 conversation-service 已有的 hash 防篡改校验保护 | — |
| 2026-06-06 | v0.36 | **SSH 连接成功态稳定性修复**：① `chat.ts` 将 `ssh_connected` 调整为候选成功，需等待短暂稳定窗口或首段 `ssh_output` 后才将全局状态置为 `connected`，避免远端 Shell 立即退出时误报成功；② `SshConnectDialog.vue` 在成功页监听全局 SSH 状态，若连接回落为 `disconnected/error`，取消自动关闭并切回错误态，用户可直接重试而无需刷新页面 | — |
| 2026-06-06 | v0.35 | **SOP 变量确认卡片选项渲染支持（PR #406）**：`MessageBubble.vue` 的 `variable_confirm` 卡片在检测到 `options` 列表长度大于 0 时，自动隐藏分栏对比面板，直接呈现 `InteractiveOptions` 单选按钮组，替代原有的文本输入模式 |
| 2026-06-05 | v0.34 | **SSH 凭证自动保存与连接关闭状态保护（PR #402）**：① `SshFormSection.vue` 为主机/用户名/密码输入框补充标准 `name` 和 `autocomplete` 属性（`name="host"`、`autocomplete="on"`、`autocomplete="username"`、`autocomplete="current-password"`），使浏览器密码管理器能正确保存和自动填充 SSH 凭证；② `SshConnectDialog.vue` 在 `handleCancel()` 中增加 `viewState !== 'success'` 保护，防止连接成功后关闭弹框时错误触发 `disconnectSSH()` 掐断已有长连接 |
| 2026-06-06 | v0.33 | **SSH 连接 Bug 修复与排查阶段名称对齐（PR #400）**：① 修复 SSH 连接弹窗提示成功后弹窗不消失且实际未连接的 Bug（`SshConnectDialog.vue` / `chat.ts` 竞态状态同步问题）；② 更新 `DiagnosticProgress.vue` 中的诊断进度条阶段名称，使之与 Agent 实际诊断阶段完全对齐 | — |
| 2026-06-05 | v0.32 | 修复 Prompt 编辑弹窗布局与保存 500 问题（PR #399）：① 修复 Prompt 管理页面编辑弹窗中模板编辑框 flex 伸缩被挤压的布局缺陷；② 修复后端 `conversation-service` 中 Prompt 保存接口使用标准 `logging` 导致 structured kwargs 触发 HTTP 500 的问题。 | — |
| 2026-06-05 | v0.31 | 变量输入与确认卡片 UI 渲染优化（PR #398）：① 前端 `InteractiveRequestCard.vue` 和 `MessageBubble.vue` 支持使用 `event.title` 动态渲染卡片标题，代替硬编码的“❓ 信息确认”；② 对 `variable_input`/`variable_confirm` 类型事件根据 metadata 动态展示必填/选填标识及格式验证占位符，并将提交按钮文案动态调整为“提交变量值”；③ 修复 `audit_log`/`tool_result` 数据库表未导入导致的落库失败问题，以及 SOP 变量提取和三路合并策略覆盖缺陷。 | — |
| 2026-05-31 | v0.30 | 落地方案 B 结构化元数据渲染引擎（PR #372）：① 彻底废除前端 `MessageBubble.vue` 脆弱的正则表达式文本扫描解析；② 前端重构为纯 `message.metadata` 驱动，当 `kind === 'choice_options'` 时百分之百高保真稳定渲染为按钮与 +1 症状描述框；③ `chat.ts` 捕获 metadata SSE 并追加到响应式仓库中；④ 后端 `conversation_service` 拦截并转化 S0 交互事件，支持 `save_assistant_message` metadata 传导并在 PostgreSQL 数据库消息中完美持久化。 | — |
| 2026-05-31 | v0.29 | S0 Triage 4+1 按钮引导词正则判定范围扩容（PR #371）：扩展前端 `MessageBubble.vue` 对 AI 选项引导语的匹配范围（添加“请补充”、“请问”等高频提问前缀），彻底解决部分 4+1 选项文本未能渲染为可点击按钮的问题，实现与历史交互一致的“禁用/不可选”和补充描述（+1 模式）特性 | — |
| 2026-05-30 | v0.28 | HTP S0 意图选择卡片交互修复（PR #360）：① MessageBubble.vue 特判 `intent_selection` 气泡点击，绕过 ops-agent `interactive-response` 接口，转换为带 metadata 的常规 sendMessage 发送；② chat.ts sendMessage/addUserMessage/streamAIResponse 升级支持可选 metadata 参数；③ 用户消息 metadata 持久化，刷新页面后卡片已选状态高亮恢复 | — |
| 2026-05-21 | v0.27 | 助手切换优先级修复：`chat.ts` `sendMessage()` 中 `assistant_type` 参数优先级由 `currentCase.assistant_type \|\| selectedAssistant` 改为 `selectedAssistant \|\| currentCase.assistant_type`，允许用户随时切换助手生效（不再强制使用工单绑定助手）| [PR #321](https://github.com/tomturing/hci-troubleshoot-platform/pull/321) |
