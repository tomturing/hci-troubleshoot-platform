...
| 日期 | 版本 | 变更内容 | 关联事件文档 |
|------|------|---------|------------|
| 2026-04-21 | v0.7 | SSH 集成创建工单：弹框弹出时 Bridge 检测；SSH 信息自动填充；一键连接创建（连接 → acli 检查 → 采集 → 终端打开）；无 SSH 创建支持（灰色按钮 + 告警提示）；环境数据折叠卡片展示；详见 [SSH集成创建工单方案](events/2026-04-21-SSH集成创建工单方案.md) | 待开发 |
