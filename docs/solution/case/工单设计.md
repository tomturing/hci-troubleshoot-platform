...
| 日期 | 版本 | 变更内容 | 关联事件文档 |
|------|------|---------|------------|
| 2026-05-30 | v2.2 | 优化工单管理高级检索与编辑功能：支持工单号、客户端ID、标题、状态、创建时间搜索（模糊搜索全支持），添加工单编辑 Dialog 双向绑定与提交逻辑，联动 API 网关执行 Pod 释放防泄露，补齐单元测试 | — |
| 2026-05-21 | v1.12 | 默认 assistant_type 更新：从 `openclaw` 改为 `htp-agent` | — |
| 2026-05-21 | v1.11 | Conversation Model 迁移：`conversation.py` 从本地定义迁移至 `shared/models/conversation.py`，与 conversation-service 共用完整定义；models/__init__.py 改为从 shared 导入 | — |
| 2026-05-20 | v1.10 | Dockerfile CACHEBUST 修复补强（PR #307）：在 `COPY shared` 前新增 `RUN echo CACHEBUST` 命令真正破坏 Docker 缓存 | — |
| 2026-05-20 | v1.9 | Dockerfile 新增 CACHEBUST ARG 解决 shared 目录缓存问题（PR #306） | — |
| 2026-05-18 | v1.8 | `_extract_task_logs` 新增 `process` 原始字段透传（字符串化），供前端基于进度值渲染进度条/终态图标；任务采集命令由 `task get -s failed -l 10` 改为 `task get -l 10`（不再仅限失败状态，支持展示进行中/完成任务） | — |
...
PUT    /api/cases/{case_id}               [Admin] 编辑工单信息
PUT    /api/cases/{case_id}/close         关闭工单并通知释放 Pod
...
GET    /api/cases/all                     Admin: 分页查询全部工单（支持高级检索与模糊搜索）
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.8 | 2026-05-20 | **可观测性模块重组（架构重构）**：`shared.utils.{logger,metrics,otel,trace_id}` 迁移至 `shared.observability.{logger,metrics,otel,trace_id}` 子包，本服务所有 import 路径同步更新（纯路径变更，无功能逻辑改动）。详见 [架构重构方案](../../solution/events/2026-05-20-架构重构-残留代码清理与模块重组.md) |
| v1.9 | 2026-07-24 | **Pydantic v2 配置迁移**：`backend/case-service/app/config.py` 的 `Settings` 由 `class Config:` 风格（`env_file`/`case_sensitive`/`extra`）迁移为 `model_config = ConfigDict(...)` 范式，对齐 pydantic v2（纯配置范式变更，无功能逻辑改动）。 |
