...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| 7.5 | 2026-06-03 | 模型下沉：将 SystemPrompt ORM 模型下沉到 shared 模块，并在 shared/models/audit.py 中导入，解决微服务间（如 agent-service）导入 AuditLog 时由于缺失引用的 system_prompt 表而引发的 SQLAlchemy 外键元数据编译异常 |
