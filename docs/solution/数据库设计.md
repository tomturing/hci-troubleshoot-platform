...
| 7 | `terminal_operation` | 工单模块 | SERIAL | 终端操作录制（命令输入/输出回显） |
| 8 | `conversation` | 对话模块 | UUID | 对话会话，含 P4 诊断 S0-S6 状态机（**核心**） |
| 9 | `message` | 对话模块 | UUID | 聊天消息，四种角色 |
| 10 | `diagnostic_item` | 对话模块 | UUID | 诊断结论条目（替代旧 hypothesis JSONB） |
| 11 | `tool_result` | 对话模块 | VARCHAR(36) | 工具调用执行结果 |
| 12 | `audit_log` | 对话模块 | VARCHAR(36) | System Instructions 构建审计 |
| 13 | `system_prompt` | 对话模块 | SERIAL | Prompt 模板库（版本化管理）（**定义**） |
| 14 | `tool_definition` | 对话模块 | SERIAL | 工具知识库（ReAct 可用工具清单）（**定义**） |
| 15 | `kb_category` | 知识库模块 | SERIAL | 分类树（235 节点），全局分类枢纽（**核心**） |
| 16 | `kbd_entry` | 知识库模块 | BIGSERIAL | KBD 知识条目（整条 embedding，无分块） |
| 17 | `sop_document` | 知识库模块 | SERIAL | SOP 文档（~20,000 字/个） |
| 18 | `sop_chunk` | 知识库模块 | SERIAL | SOP 分块检索 |
...
*文档版本: 6.7 | 更新日期: 2026-05-19 | 18 张表全量覆盖，三模块架构（工单/对话/知识库）| 新增 sop_tree 表（SOP 多叉决策树存储）*
