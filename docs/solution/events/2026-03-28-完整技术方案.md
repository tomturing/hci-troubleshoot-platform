...
Phase A（当前，并行进行）：S0 意图识别 + 分类基线重构
  → 扩展 kb_category 表（补充 hit_count / is_active 字段）
  → S0 阶段注入 198 分类列表，约束 LLM 输出固定格式
  → 后端 regex 提取 category_id（对应 kb_category.code）写入 conversation
  → S1+ 阶段用 category_id 直查 SOP（三轨串行路由：SOP → KBD → 人工）
  → Admin-UI 分类管理页面（查看 / 编辑 / YAML 导入导出）
  → 详细方案：[22_S0意图识别与分类基线重构方案.md](./22_S0意图识别与分类基线重构方案.md)
