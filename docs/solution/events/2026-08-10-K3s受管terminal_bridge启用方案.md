...
- Deployment、Service、Ingress、单副本门禁和 NetworkPolicy 已纳入 Helm/GitOps；最终现场没有依赖手工端口转发。
- Chromium 通过同源 WebSocket 连接受管 Bridge；Bridge 使用 K3s DNS 连接 hci-sim，并将 `case_id/conversation_id/trace_id/exec_id` 传递到结构化日志。
- KBD 27123 的 task、lsof、ps 均由 Agent 发起，三条命令在 Bridge 和 hci-sim 两侧精确对应且 exit code 0；随后 Result 持久化为 passed。
- 本方案保持单副本。多副本所有权迁移和容量故障注入仍是后续 P2，不能通过增加 replicaCount 绕过。
