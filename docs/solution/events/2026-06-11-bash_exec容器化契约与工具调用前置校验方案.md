...
2. **受控运行时适配器**：对无法映射为 aCLI system 的 Bash 片段，由服务端 `ContainerExecAdapter` 按目标 HCI 版本选择容器运行时进入方式。LLM 不感知也不能控制该细节。
3. **无法确定运行时**：fail-closed，返回校验失败，要求 LLM 改用 `acli_exec` 或请求用户补充环境信息。
...
- `scripts/tools/sync_acli_catalog.py` 同步更新 agent-service 与 kb-service 两份 catalog 快照，保证发布校验和运行时校验使用同一来源。
