...
AI 后处理的系统约束已纳入现有 Prompt 管理体系，不再由运行代码拼接硬编码文本：

- Prompt 名称：`ai_processing_v1`
- 管理阶段：`AI · 数据后处理`
- 存储：`system_prompt` 表；初始版本由 `database/atlas-migrations/20260827000000_seed_ai_processing_prompt.sql` 和 `database/seeds/02_system_prompts.sql` 幂等写入。
- 逻辑槽位：运行代码引用 `ai_processing`，由 `prompt_slot` 指向当前激活的具体版本；新增或切换 AI 阶段版本时，管理 API 同步更新槽位，避免代码固定绑定某个版本名。
- 管理：复用 conversation-service 的 `/api/v1/prompts` 增删改查、版本和激活切换接口；Admin UI 的 Prompt 管理页面新增 AI 阶段。
- 热生效：每次 AI 后处理调用都通过 `StrictPromptLoader.load_and_validate` 读取当前激活版本。加载器同时调用 `DynamicResourcePublisher` 登记 revision，因此不需要重启 agent-service；Prompt 缺失、占位符错误或数据库不可用时 Fail Closed。
- 模板占位符：仅允许 `{mode}` 和 `{output_type}`，由平台渲染；用户业务说明仍来自每个信号的 `ai_processing.instruction`，不会写入系统模板。
- 一致性：QFK 与 QKV 通过同一个 `extract_ai_value` 调用同一 Prompt；平台统一记录动态资源 revision、调用链和响应摘要，支持回放与回滚。

对抗性审查结论：系统 Prompt 与用户 instruction 分离，避免用户配置覆盖平台安全约束；激活切换只影响后续调用，正在执行的调用使用已加载的模板快照；旧版本仍保留在 `system_prompt` 和动态资源 revision 中，可通过管理页面切换恢复。
...
- QKV 派生变量单元（OutputProcessingEditor）中，`item.type` 与 `ai_processing.output_type` 统一共用同一份配置，文案统一为“输出类型”，消除双重类型下拉框；AI 模式下“处理方式”统一为下拉框并与提取方式和输出类型三列等高对齐。
...
本次实施已完成统一契约模块、QFK AI 响应校验、QKV 串行执行调整、Schema 和前端字段迁移与 OutputProcessingEditor 交互收敛；提交前执行 `uv run ruff check`，并按上述验收标准补充单元与集成测试。
