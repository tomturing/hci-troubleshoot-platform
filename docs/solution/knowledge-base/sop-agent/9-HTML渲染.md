...
HTML 作为静态标记语言，本身不具备交互能力。如何在纯前端条件下实现搜索、引导向导、步骤导航等交互功能？

## 核心思路

采用 **Jinja2 模板 + JavaScript 运行时** 的混合架构。模板负责渲染 HTML 结构，JavaScript 运行时提供搜索、引导向导、步骤导航等交互逻辑，两者完全运行在浏览器端。

## 核心目标

1. 将 SOP JSON 转换为可交互的纯静态 HTML 页面
2. 支持搜索、引导向导、图片灯箱等交互功能
3. 亮暗主题切换与响应式布局适配多端
4. 多节点时自动生成索引页

## 核心约束

1. **无服务端依赖**：所有交互逻辑必须在浏览器端实现
2. **离线可用**：图片等静态资源需内联或下载缓存
3. **多节点索引**：多节点渲染时必须生成索引页

## LLM 使用情况

**使用**：否

此阶段为纯模板渲染，依赖 Jinja2 模板引擎和 JavaScript 运行时，不调用任何 LLM API。
