...
- **模式切换与按钮控制**：支持“结构化卡片视图”与“JSON 源码编辑器”双模式切换；【保存并热生效】按钮仅在源码编辑器模式下展示，结构化视图下隐去以提升流畅度。
- **删除防误触二次确认**：结构化视图中的“删除”操作增加 ElMessageBox 防误触二次弹窗确认，点击确认后自动触发后端 REST API 并实时热重载。
