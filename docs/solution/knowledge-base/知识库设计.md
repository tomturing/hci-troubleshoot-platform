...
> - **相关文档**：[../ai-assistant/AI助手设计.md](../ai-assistant/AI助手设计.md) — AI 层如何消费知识库；[../接口设计.md](../接口设计.md) — KB Service REST API 规范；[../events/2026-03-09-AI层RAG对接架构决策.md](../events/2026-03-09-AI层RAG对接架构决策.md) — AI 层与 RAG 层对接决策。
...
> 详细决策依据见 [2026-04-02-S0意图识别与分类基线重构方案.md](../events/2026-04-02-S0意图识别与分类基线重构方案.md)。
...
- **Conversation Service** 调用 `/api/kb/search` 后，在 AI 层进行 4-Tier Prompt 组装，详见《AI助手设计.md》
- **LearningClaw** 调用 `/api/kb/ingest` 将学习成果写入知识库，详见《AI助手设计.md §第二部分》
...
flowchart LR
    ...
    MD -->|"POST /api/kb/ingest"| KBS
    SOP -->|"POST /api/kb/ingest?type=sop"| KBS
    SPLIT --> EMBED --> IDX
    IDX -->|"INSERT kbd_entry (vector)"| KBE
    IDX -->|"INSERT sop_chunk (vector)"| SOPC
    KBC -.->|"category_id 关联"| KBE & SOPD
```
...
```mermaid
flowchart TD
    CVS["Conversation Service"] -->|"POST /api/kb/search\n{query, top_k=5, category_id?}"| KBS["KB Service"]

    KBS --> BM25["BM25 关键词检索\n(pg_trgm 支持)"]
    KBS --> VEC["pgvector 语义检索\n(cosine similarity)"]
    BM25 & VEC -->|"混合得分 RRF 融合"| RERANK["重排 top-k"]
    RERANK -->|"chunks ~3000 tokens"| CVS

    CVS -->|"POST /api/kb/sop/match\n{query}"| KBS
    KBS -->|"TF-IDF + 关键词分类器"| SOPM["SOP 匹配\n关键词路由"]
    SOPM -->|"matched SOP 全文\n~2000 tokens 或 null"| CVS
```
...
```mermaid
flowchart LR
    subgraph PROMPT["messages[] 发给 AI Pod"]
        T1["Tier-1 System Prompt\n~500 tokens\n角色 + 准则"]
        T2["Tier-2 SOP 全文\n~2000 tokens (可选)\n关键词匹配后注入"]
        T3["Tier-3 RAG Chunks\n~3000 tokens\n语义检索 top-5"]
        T4["Tier-4 Chat History\n~4000 tokens\n最近 20 条"]
        T5["Current User Message"]
    end

    SOPD[("sop_document")] -->|"全文"| T2
    KBE[("kbd_entry")] -->|"向量检索"| T3
    MSG[("message")] -->|"历史"| T4
    SP[("system_prompt")] -->|"激活模板"| T1
```
