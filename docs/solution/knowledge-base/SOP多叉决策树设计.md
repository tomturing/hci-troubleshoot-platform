...
| 日期 | 版本 | 变更内容 | 关联事件文档 |
|------|------|---------|------------|
| 2026-05-27 | v1.6 | PrerequisiteItem 字段重构遗留 bug 修复（PR #338）：修复 sop_parser.py 中 condition→description 字段引用导致发布 HTTP 500 | — |
| 2026-05-27 | v1.5 | SOP 解析器字段重构遗留 bug 修复（PR #337）：修复 sop_parser.py 中 SOPNode 字段引用（name→title, node_id→id）导致发布 HTTP 500 | — |
...
前端决策树展示层同时兼容历史 `tree_json`：如果旧数据中已经保存为
```bash / 命令正文 / ``` 三条记录，渲染时会临时合并为一条命令展示，
避免必须重新发布旧 SOP 文档才能看到正确效果。

判断方法展示层会对 `acli_methods` 做轻量命令识别：真实 shell/acli 命令使用终端样式，
说明性文字（如“回显中带有...”）使用普通说明样式，避免将排障说明误当命令。
带自动编号的列表展示前会剥离原文中的 `1、` / `1.` 等编号前缀，避免出现双编号。
