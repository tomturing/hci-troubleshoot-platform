...
> 本文只维护单 Signal 试运行与 Bundle 验证资产。整体文档入口见 [KBD 与关键信号导航](README.md)，案例发布操作见[审核手册](../../user-guide/KBD审核与信号配置-使用手册.md)。无信号参考案例不需要为发布补做 Signal 试运行。

## 变更历史

| 日期 | 变更 |
|---|---|
| 2026-09-23 | 接入知识库统一导航，明确试运行适用范围与无信号推荐的边界；既有执行规则不变。 |

## Admin UI 结果展示约定

试运行对话框采用双列响应式布局（左侧提供验证范围选择与样本输入，右侧展示统一的「运行结果」面板，底部提供「试运行」触发与「保存到 Bundle 草稿」操作）。
