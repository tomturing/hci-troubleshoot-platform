...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| **9.16** | **2026-06-17** | **ReAct 工具调用历史跨轮次持久化（PR #472）**：① `backend/shared/models/schemas.py` MessageRole 枚举增加 `TOOL_CALL`/`TOOL_RESULT`；② agent-service 新增 `_persist_tool_turn()` fire-and-forget 持久化；③ conversation-service `/tool-turn` 接口与 `history_messages` 重建逻辑；④ 滑动窗口压缩防止 token 爆炸 | [events/2026-06-17-ReAct工具调用历史跨轮次持久化.md](agent/events/2026-06-17-ReAct工具调用历史跨轮次持久化.md) |
