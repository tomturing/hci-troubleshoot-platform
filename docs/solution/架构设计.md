...
| **9.35** | **2026-08-03** | **关键信号 timeout 单一默认值**：shared `acquire.args.timeout` 以 120 秒作为默认值，Agent 的预览、CDD 编译和 Differential 执行均引用同一来源；显式 timeout 仍优先，避免显示、命令模板与运行时分叉。 | PR #663 |
