...
| 版本 | 日期 | 变更内容 | 关联事件文档 |
|------|------|---------|------------|
| **9.20** | **2026-07-09** | **KBD 分类与识图 Prompt 管理与重算链路修复**：① 修改 `PromptManageView.vue` 增加 `KBD` 阶段以打通模板管理；② 修复 `api-gateway` 代理中未声明 `timeout` 参数导致识图接口 500 TypeError 错误，并提高重分类请求超时至 120s 解决网关 30s 超时 HTTP 503 错误；③ 将 `kb-service` 模型回退默认值变更为支持 Vision 多模态的 `qwen3.7-plus`，解决 `glm-5` 无法识图的问题。 | [events/2026-07-09-KBD分类与识图Prompt管理页显示修复方案.md](events/2026-07-09-KBD分类与识图Prompt管理页显示修复方案.md) |
| **9.19** | **2026-07-09** | **使用 LLM_DEFAULT_MODEL 代替未注入的 CLASSIFY_MODEL/VISION_MODEL（PR #512）**：解决由于 Helm 中未注入 `CLASSIFY_MODEL` 和 `VISION_MODEL` 导致服务回退到无效的 `qwen3.7-plus` 模型名，进而引发 HTTP 503 和 HTTP 500 报错的问题。统一改用已注入的全局默认模型 `LLM_DEFAULT_MODEL` (`glm-5`)。 | — |
