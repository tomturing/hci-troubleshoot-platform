...
| **6.4.3** | **2026-04-07** | **路由冲突+Token修复**：删除 classify.py 死代码端点 `GET /categories`（零调用者）消除与 categories.py 的路由冲突；api-gateway 代理改用 `_internal_auth_headers()` 注入网关自身 Token，不再透传浏览器 Token |
...
graph TB
    ...
    subgraph AI["AI 层 (AI Assistant Layer)"]
        OCP["OpenClaw Pool\n:18789"]
        NBP["NaboBot Pool\n:8080 (计划)"]
        EXT["可扩展: PicoClaw / 自研助手\nAssistantRegistry 配置化注册"]
    end
...
sequenceDiagram
    ...
    participant AI as AI Pod (OpenClaw)
    ...
    GW->>CVS: POST /internal/chat {case_id, content, trace_id}

    CVS->>DB: INSERT message (role=user)
    CVS->>DB: SELECT conversation history (最近20条)

    par 知识库并行检索
        CVS->>KBS: POST /kb/search {query, top_k=5}
        KBS-->>CVS: RAG chunks (~3000 tokens)
    and SOP 场景匹配
        CVS->>KBS: POST /kb/sop/match {query}
        KBS-->>CVS: matched SOP 全文 (可选, ~2000 tokens)
    end

    CVS->>SS: allocate_pod(case_id, assistant_type)
    SS->>Cache: GET pod:openclaw:idle
    Cache-->>SS: pod_endpoint
    SS-->>CVS: pod_endpoint

    CVS->>CVS: 组装 messages[]\n[system | sop | rag | history | user_msg]
    CVS->>AI: POST /v1/chat/completions {messages, stream=true}
    ...
    CVS--)+SS: release_pod(pod_id) [BackgroundTask]
```
...
*文档版本: 6.3 | 创建: 2026-02-15 | 更新: 2026-04-04*
