...
| **9.47** | **2026-08-06** | **QKV/QFK 关键词参数类型边界**：共享 `acquire.args` 门禁保持 `qkv_*` 关键词为单个 `string`，新增稳定错误码；Prompt v2.3 明确数组仅属于 `match.pattern` 或 `extract.rows.include/exclude`，多个任务动作拆成多条 Candidate，避免 LLM 字段语义串线。 | [QKV 关键词类型契约收敛方案](knowledge-base/events/2026-08-06-KBD信号QKV关键词类型契约收敛方案.md) |
