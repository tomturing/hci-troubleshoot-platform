...
| C | Resolver/Compiler、Registry/Storage、审批/stale/provenance | 控制面内核和 desired schema 已实现；权威数据源、持久 CAS、对象存储待接入 | [阶段 C](agent/events/2026-08-05-hci-sim阶段C-Fixture编译与注册控制面方案.md) |
...
当前事实边界：历史 KBD 27123 只有单轮 P0 Golden E2E；当前没有 Fixture Compiler、Scenario/TestRun API、Scheduler 或 `agent_test_*` 表，当前 dev context 也未发现 hci-sim namespace。以上目标架构不得被解读为现有实现。
