...
sequenceDiagram
    ...
    alt assistant_type = "ops-agent"
        BR->>OA: POST /acp/sessions（幂等创建，session_id=conversation_id）
        BR->>OA: POST /acp/sessions/{id}/prompt {user_message}
        loop SSE 流式消费
            OA-->>BR: ACP event (text_delta / session_done)
            alt event = BrainInteractiveRequest（需人工确认）
                BR-->>GW: 透传交互请求
                GW-->>UI: 推送确认弹窗
                用户-->>UI: 填写并提交
                UI->>GW: POST /api/conversations/{id}/brain-response
                GW->>CVS: 转发 brain_response
                CVS->>BR: submit_acp_response()
                BR->>OA: POST /acp/sessions/{id}/responses/{req_id}
            else event = text_delta
                BR-->>CVS: BrainEvent (token chunk)
                CVS-->>GW: SSE event
...
│  │  多 session 隔离：各 ACPSession 独立 llm_message_history        │    │
...
> - **ops-agent ACP REST 集成（方案E）**：✅ v2.9.0 T-E1～T-E7 全部完成。HTP 侧 `OpsAgentBrainAdapter` 完全走 ACP REST；ops-agent 侧 `server/main.py`（FastAPI + uvicorn）已在部署镜像中实现（本地 workspace `ops_agent/server/` 为空是本地代码未同步问题）。
