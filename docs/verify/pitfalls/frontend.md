...
## V-013：可编辑列表的渲染 key 不能依赖正在编辑的字段

**现象：** 输入框每输入一个字符就丢失焦点，只能逐字重新点击输入。

**根因：** `v-for` 的 `key` 拼接了输入框当前值；受控值每次更新都会改变 key，Vue 将节点视为新对象并重建 DOM，浏览器焦点随旧节点销毁。

**修复：** key 必须来自编辑态稳定身份，且不能写入后端契约。字段更新应只替换对象内容并保留该身份；删除或新增项目也要保持其余项目的身份不变。

**预防测试：** 回归测试必须在真实 DOM 中输入至少一个字符，断言输入节点引用保持不变且可继续输入；同时覆盖删除前置项目后剩余项目仍能继续编辑。
