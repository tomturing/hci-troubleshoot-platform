...
OpenClaw 以 systemd 用户服务运行在宿主机 `:18790`，不纳入 K8s 管理：

```yaml
# ExternalName Service — 让集群内 Pod 通过 DNS 访问宿主机 OpenClaw
apiVersion: v1
kind: Service
metadata:
  name: openclaw
...
spec:
  type: ExternalName
  externalName: host.k3s.internal  # K3s 自动解析到宿主机 IP
  ports:
    - port: 18790
```

**设计原因：**
- OpenClaw 是 Node.js 应用，已通过 systemd 稳定运行，无需容器化
- ExternalName 避免硬编码 IP，Pod 通过 `openclaw.hci-troubleshoot.svc.cluster.local:18790` 访问
- 后续迁移 OpenClaw 到 K8s 时只需替换此 Service 定义，上游无感
...
| openclaw | ExternalName | 18790 | — | 宿主机 OpenClaw |
...
| AI 层 (OpenClaw) | ExternalName Service | 暂不纳入 K8s 管理 |
...
| OPENCLAW_BASE_URL | docker-compose.yml env | ConfigMap | `http://openclaw:18790` |
