...
### 影响范围
- data-pipeline/kbd/ 目录下的所有 Pipeline 脚本
- 知识库数据导入效率
- 运维排障效率
...
### FR1: 日志文件持久化
- 日志保存路径：`data-pipeline/kbd/logs/kbd_{run_id}.log`
...
### FR2: 进度 JSON 追踪
- 进度保存路径：`data-pipeline/kbd/logs/progress_{run_id}.json`
- 进度结构：
  ```json
  {
...
- [ ] 标准1：运行 `pipeline --ids 15414 --stages fetch` 后，`data-pipeline/kbd/logs/kbd_*.log` 文件存在且包含完整日志
...
### 依赖约束
- 需确认 `data-pipeline/kbd/logs/` 目录权限
