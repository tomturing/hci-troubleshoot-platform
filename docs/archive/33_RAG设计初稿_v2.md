...
│ ↓                                                                       │
│ 上下文组装 → Conversation Service → AI 回答用户                           │
└─────────────────────────────────────────────────────────────────────────┘
                                 │
                            用户对话结束
                                 │
...
```text
┌─────────────┐ ┌─────────────────┐ ┌──────────────┐
│ API Gateway │────▶│ KB Service │ │ PostgreSQL │
│ (8000) │ │ (8004) │────▶│ (pgvector) │
└─────────────┘ └────────┬────────┘ └──────────────┘
│
┌────────┴────────┐
...
- Conversation Service 新增 kb_client.py — HTTP 调用 KB Service /api/kb/search
- conversation_service.py 新增 assemble_context() — 4-Tier prompt 组装
- API Gateway 注册 /api/v1/kb/ 路由
- docker-compose.yml 新增 kb-service
