...
| 日期 | 版本 | 变更内容 | 关联事件文档 |
|---|---|---|---|
| 2026-07-28 | v1.1 | 关键信号主报告改为用户态状态与结构化产出；27123 第三步以定向 `cmd=` 非空唯一结果确认进程存在，不依赖特定进程名 | [KBD关键信号结果展示与ps输出提取方案](../../../../solution/events/2026-07-28-KBD关键信号结果展示与ps输出提取方案.md) |
| 2026-08-20 | v1.2 | 校正当前 CDD 调度事实：分类清单不接收 KBD `top_k`；候选不因早停、采集器频次或仿真 TestRun 标签被截断 | [仿真诊断等价性与 CDD 全候选修正方案](../../../../solution/agent/events/2026-08-20-仿真诊断等价性与CDD全候选修正方案.md) |
| 2026-07-26 | v1.0 | S0 分类权威输入、分类全量 KBD、CDD 证据闭环初版 | — |

> 主动信号选择、采集复用、候选状态机和四级结论门禁的详细算法见
> [KBD 主动诊断信号调度与证据闭环算法设计](../../../../solution/knowledge-base/2026-07-26-KBD主动诊断信号调度与证据闭环算法设计.md)。
...
关联复盘：[Q2026072624224：S0 到 S1 知识路由与 CDD 失效分析](../../../../solution/knowledge-base/events/2026-07-26-Q2026072624224-S0到S1知识路由与CDD失效分析.md)。
...
15. **主机路由归一**：QKV `task/alert` 返回的 `host` 是节点名称，写入变量池前必须通过同一会话执行 `acli --formatter json platform node list` 解析为节点 IP；后续 QFK 优先使用该 IP 路由，解析失败须保留原值并记录告警。
16. **QFK 单一结果语义**：每个 QFK 必须且只能选择“关键字判定（`match`）”或“产出变量（`orchestrate.produces`）”。后者在命令成功后把完整命令输出（空 `path` / `stdout`）或 JSON 点路径（支持 `|` 回退）写入变量池，供下游 `{{变量名}}` 使用。
17. **QFK 执行位置与超时真实透传**：`qfk_system.acquire.args.container=host` 表示 terminal bridge 不包装 `container_exec`、直接在宿主机执行；其他允许容器由 bridge 包装执行。`acquire.args.timeout` 限制为 1–300 秒，必须从 Agent → conversation-service → SSE/Custom UI → terminal bridge 透传，桥端超时会关闭独立 SSH session 并返回 timeout；Redis 等待时间仅保留回传余量，不能提前截断真实超时。
...
3. `sig_001` 实际查询失败任务并产生 HOST、VM 等变量；HOST 从 `SVR_aCloud_668` 归一化为节点 IP（如 `172.28.24.2`）。
...
|---|---|---|---|
| `sig_001/qkv_task` | `489834d8-d5b0-5380-a03c-604c90b0f003` | 从任务 907 获取 VM `4359974862144`、节点名称 `SVR_aCloud_668`，再通过 `platform node list` 解析 HOST IP | PASS |
...
完整现场分析、两工单证据和 UI 模板见 [KBD关键信号结果展示与ps输出提取方案](../../../../solution/events/2026-07-28-KBD关键信号结果展示与ps输出提取方案.md)。
