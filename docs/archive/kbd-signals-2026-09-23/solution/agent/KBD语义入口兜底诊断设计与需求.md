...
> 2026-09-23 补充：[无信号案例推荐与诊断能力分离](../../../../solution/agent/KBD无信号案例推荐与诊断能力分离.md) 增加标题/问题描述推荐入口。画像对纯知识推荐不再必填；下文的强证据门禁继续约束自动执行，采集不可用时允许展示明确标注为未验证的参考案例。
...
| `matched_inconclusive` | 强生产者返回过记录，但完整 CDD 仍无法形成确定性结论 | 只允许展示 `guidance_only` 人工补证据指引；禁止执行语义候选消费者 |
...
flowchart TD
    ...
    F -->|过多或缺信息| I[有界追问或转人工]
    C -->|继续验证| G
    C -->|CDD 仍不确定且命中人工指引画像| H
    G --> J[按真实证据支持或排除，不按排名裁决]
```

在线继续使用既有 Agent、QFK、terminal_bridge（终端桥接）、命令确认、超时和审计。仿真只切换采集结果提供方，不用仿真案例标签预先指定答案。

分类快照中的“可执行”标记必须与 CDD 编译门禁一致。已发布 KBD 若 `generation_metadata.status=stale`，表示其信号生成依据已过期，必须在快照阶段排除出自动执行和强生产者完整性计算；不能先把它的任务、告警或弹框信号算作“必须查询”，再在运行时拒绝执行，从而把无关工单误判为强生产者不可用。

CDD 对同一 KBD 的必要信号采用剪枝：一条必要信号已被反证后，后续信号可以不再调度。强生产者可用性只检查实际执行过的强信号是否有错误、阻断或未知结果；不得把这类已被正确剪枝的后续信号误作查询不完整。

`guidance_only` 返回补证据请求后即结束当前诊断轮次，不再附加通用 CDD 报告或升级人工卡片。后续客户补充沿用会话中已持久化的补证据上下文，只重新解析语义画像，不重复执行全量 CDD。CDD 重试写入 `diagnostic_item` 时按 `(conversation_id, type, seq)` 幂等更新，避免审计唯一键冲突影响后续对话。

客户侧应明确展示语义命中的 Support ID 和案例标题，并标注“仅提供补证据方向，尚未确认根因”。补证据项由画像中的声明式请求驱动；通用层不得为某个案例硬编码关键词，也不能因用户消息中只出现某个短词就误认为包含多字段的证据请求已完成。需要自动判断完整性时，应在画像中另行声明结构化证据字段，由表单或受控提取器提交。

结构化字段使用 `manual_evidence_fields`，每项至少包含稳定的英文 `id` 与客户可见的 `label`，并可声明 `required`、`input_type`（`text` 或 `textarea`）和提示文字。客户侧以 `semantic_evidence_response` 连同候选 KBD ID 与字段值提交；服务端只接受画像中已声明的字段，按必填字段是否齐全决定下一轮仍需补什么。该协议适用于所有 `guidance_only` 画像，不把案例名、产品名或某个故障关键词写入通用代码。旧 `manual_evidence_request` 仅用于未迁移画像的文字兼容，不能作为字段完成度的自动依据；专家应逐步将其拆分为原子字段。

`guidance_only` 画像还可以显式声明 `semantic_recommendation`。强生产者未形成确定性支持时，路由器只在第一名达到 `minimum_score`、且与第二名分差达到 `minimum_margin`、未命中排除锚点时，返回唯一的高置信语义推荐。该推荐必须标注“来自工单描述、未经过现场采集验证”，不推进 S4，也不替代消费者验证；没有此声明的画像仍只进入补证据或人工复核路径。

推荐结果展示被选中 KBD 已审核的根因与处理建议，同时呈现触发推荐的画像锚点和澄清事实。人工补证据字段在推荐达成后转为可选核验资料，不再阻塞结论卡；它们仍可为后续现场验证补充可审计材料。

当推荐门槛尚未满足时，画像可声明 `semantic_disambiguation`。每个问题都有稳定 ID、客户可见问题和至少两个稳定选项；选项的 `support` 或 `exclude` 效果仅作用于声明它的候选。客户侧提交 `question_id + choice_id`，服务端以该结构化事实重新路由，不用关键词从用户自由文本猜测答案。仍无法收敛时才继续下一条画像问题或转人工复核。
...
既有信号抽取 LLM 提示词及其完整流程仍见 [KBD 审核发布操作指南](../../../../solution/agent/KBD审核发布操作指南.md)。不要把本规则生成器描述成新增的模型提示词。
