...
> 文档定位：这是**现行操作口径 + 目标治理设计**的合并维护文档；凡标注“规划/建议/P0/P1/P2”的内容，均不是新的 KBD 状态，也不代表平台已经自动执行。
...
| 平台 / 流水线管理员 | 第七章、第八章、第十章、10.9 | 用批量任务替代逐篇专家语义审核 |
...
完整字段、JSON、运行边界和已执行回归记录见 [KBD 语义入口兜底诊断设计与需求](../../../../solution/agent/KBD语义入口兜底诊断设计与需求.md)。该入口不改变已有任务、告警、弹框、视觉及效果验证信号的专用安全门禁。
...
| 主题 | 当前可执行的事实 | 仍需人工补位 / 目标治理 | 责任人 |
|---|---|---|---|
| 发布候选 | 审核页「检查当前内容」与发布接口运行统一静态审查 | `publish_candidate` 报告、阻断项聚合和自动队列尚未完整产品化 | 平台管理员维护门禁；审核人处理当前 KBD |
| 运行时验证 | 可试运行单 Signal，并将 PASS 保存到 Bundle Draft | 正反 Fixture 覆盖率、资产过期自动重测仍需人工管理 | 审核人提供可信样本；仿真管理员维护 Bundle |
| 离线资源 | 已发布 KBD 可执行“增量检测 → 审查 → 批准发布” | 缺失映射修复、资源覆盖率和版本回退由离线管理员管理 | 离线诊断管理员 |
| Prompt / 工具变更 | Prompt 热加载，工具和 Signal 契约有静态审查 | 影响分析、历史 Prompt 一键复现和批量重测队列仍需按本章口径补齐 | 平台 / 流水线管理员 |

**责任边界：** 审核人员只对原文事实、信号语义、可信样本和发布决定负责。Tool Registry（工具注册表）、Prompt、外部变量供给、离线映射和仿真环境的问题，应携带 KBD ID、修订 ID、Signal ID、Trace ID（链路标识）移交相应管理员，不能在 KBD JSON 中临时绕过。
...
#### 10.6.1 停用 / 归档的跨系统退役闭环

已发布 KBD 退回草稿或归档不是只改一条状态。它必须让在线 Agent、离线资源和仿真候选集按顺序收敛，避免“页面已停用、旧 Bundle 或旧采集计划仍在使用”的假停用。

```mermaid
flowchart LR
    A[已发布 KBD 退回草稿 / 归档] --> B[写入不可变停用修订]
    B --> C[Agent Active 不再命中该 KBD]
    B --> D[离线增量检测识别 tombstone 停用标记]
    D --> E[批准同步：资源失效 / 标记陈旧]
    B --> F[Bundle 候选集移除该修订]
    F --> G[同分类 Bundle 回归]
    E --> H[旧计划 / 制品仅保留追溯，不再供新工单使用]
    G --> I[记录退役证据与责任人]
```

| 验收项 | 必须确认的事实 | 负责人 |
|---|---|---|
| KBD 生效链 | 已产生停用修订，Agent 当前生效版本不再引用该 KBD | KBD 审核人 |
| 离线资源链 | 增量检测识别停用记录；批准后新工单不会使用旧 Collector（采集器）/Collection Profile（采集画像） | 离线诊断管理员 |
| 仿真链 | 受影响 Bundle 候选集已刷新并回归；旧 Bundle 若仅为追溯应明确标为陈旧 | 仿真管理员 |
| 审计链 | 记录 KBD 修订、同步批次、Bundle Digest（包摘要）、操作人、Trace ID 和完成时间 | 各责任人 |

若任一链路尚未完成，应把 KBD 标记为“已停用、下游同步待完成”，而不是声称离线和仿真资源已同时清理完毕。
...
语义入口的单篇离线验收按 `semantic-offline-prepare → collect → upload → result → verify` 五阶段执行，对应 Make 目标均需传 `RUN_ID`、`SUPPORT_ID`（案例编号）和 `INSTANCE`（仿真实例名）。需客户确认的变量通过 `SEMANTIC_OFFLINE_ARGS='--known-variable NAME=VALUE'` 传入，不能由脚本猜测。原全量五篇优先使用 `signal-v2-offline-matrix` 一次跑完并保存每篇结果。详细隔离边界、两套五篇在线／离线结果和故障排查见 [KBD 语义入口兜底诊断设计与需求](../../../../solution/agent/KBD语义入口兜底诊断设计与需求.md#9-本次验证记录与上线检查)。
...
| 审计证据 | 用途 | 当前获取位置 / 操作 |
|---|---|---|
| KBD ID、Proposal Revision ID、生成时间 | 锁定被复核的知识和版本 | KBD 修订历史 |
| `generation_metadata.prompt_revision`、模型 ID、输入/工具契约指纹 | 判断提示词、模型、原始输入和契约是否已变化 | Proposal 的 `signals_json.generation_metadata` |
| Trace ID | 关联 LLM 调用、动态资源使用审计和运行日志 | 抽取批次 / KBD 修订审计 |
| Prompt 名称、版本、动态资源快照 ID 与内容摘要 | 找回当时实际加载的 Prompt 版本 | Prompt 动态资源审计；变更发布记录 |
| 渲染输入的来源摘要 | 判断是正文、截图事实、目录还是变量表发生变化 | 抽取审计 / Langfuse（可观测平台）Trace |

当前 `generation_metadata` 已保存 Prompt 哈希等指纹，但不应假设它天然携带完整 Prompt 文本或快照 ID。若审计界面无法按 Trace ID 取回动态资源快照，平台管理员应把该次生成标记为“**可比对指纹，不能完全复现**”，并在下一次 Prompt 发布前补齐快照引用；不能仅凭今天查询到的 `content_template` 宣称它就是历史生成所用版本。
