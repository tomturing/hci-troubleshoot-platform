...
| 知识类型 | 代表 | 知识性质 | AI Agent 使用方式 |
|---------|------|----------|-----------------|
| **SOP（排障手册）** | `vm_start_failure/chapters/` | 程序性知识（How-to） | 按决策树遍历，执行命令，在判断节点做决策 |
| **KB案例（历史故障）** | `cases_raw/cases.jsonl` | 陈述性知识（What-happened） | 从 `root_cause` 字段提取假设，`solution` 作参考 |
...
**最终方案：C 为主框架（SOP轨道优先，KB轨道兜底），B 为 KB 轨道检索引擎**
...
```text
  ...
  │                    数据源 (3 种入口)                      │
  ├──────────────────┬──────────────────┬────────────────────┤
  │ S1: 在网KB 7000+ │  S7: 增量新案例   │  S9: 客户端即时生成  │
  │    (批量 ETL)    │   (定期采集)      │ (随工单关闭触发)     │
  └────────┬─────────┴────────┬─────────┴──────────┬─────────┘
           │                  │                    │
           ▼                  ▼                    ▼
  ...
  │                                                          │
  │  status: draft                                           │
  │  存储: draft/ 目录 + kb_document(status='draft')          │
  │  可见性: 仅 Admin 后台可见                                 │
  │  操作: 浏览 / 批注 / 编辑 / 批量通过 / 拒绝                 │
  └──────────────────────────────┬───────────────────────────┘
                                 │
  ...
  │                                                          │
  │  status: published                                       │
  │  文件双写: published/ 目录 (可 Git 管理, 支持 diff)        │
  │  DB双写:  kb_document → 触发分块 → embedding → 索引       │
  │  可见性: 检索可用 + Admin 可管理                           │
  └──────────────────────────────┬───────────────────────────┘
                                 │ 触发向量化
                                 ▼
  ...
  │  embedding    向量索引 (pgvector IVFFlat, cosine)         │
  │  tsvector     全文索引 (BM25, jieba 分词)                 │
  │  kb_sop_node  SOP 关键字路由表                            │
  │  kb_category  分类树 (4层, 6个L1)                         │
  │                                                          │
  │  特性: 全部可从 published/ 全量重建                        │
  └──────────────────────────────┬───────────────────────────┘
                                 │
                                 ▼
  ┌──────────────────────────────────────────────────────────┐
  │            ④ KB Service 检索引擎 (运行时)                 │
  │                                                          │
  │  LLM分类 → SOP精确匹配 → 混合检索(BM25+Vector+RRF)         │
  │          → Rerank → 返回 ranked results                  │
  │                                                          │
  │  ⚑ KB Service 职责止于「返回」                            │
  │    上下文组装与 LLM 生成属于 AI 层职责                     │
  │    详见《12_AI层设计.md》                                 │
  └──────────────────────────────┬───────────────────────────┘
                                 │ 用户对话结束
                                 ▼
