...
- **依据文档**: `20_数据库重规划分析.md`（目标 11 张表）、`21_知识库模块重设计.md`（方案B）
...
| **5.0** | **2026-04-02** | **重大重构**：基于 21_知识库模块重设计.md（方案B），目标从 17 张精简为 **11 张**；移除"数据管道"模块；知识库只保留 4 张表；合并审计表为 `audit_log` |
...
> **依据**：`20_数据库重规划分析.md` v2.0

| # | 表名 | 所属模块 | 主键类型 | 说明 | 状态 |
|---|------|---------|---------|------|------|
| 1 | `user` | case-service | UUID | 用户身份（临时/认证） | ✅ 保留 |
| 2 | `case` | case-service | VARCHAR(20) | 排障工单，状态机驱动 | ✅ 保留 |
| 3 | `environment` | case-service | UUID | 客户现场环境采集数据 | ✅ 保留 |
| 4 | `conversation` | conversation-service | UUID | 对话会话，含 P4 诊断状态机 | ✅ 保留 |
| 5 | `message` | conversation-service | UUID | 聊天消息四种角色 | ✅ 保留 |
| 6 | `audit_log` | conversation-service | VARCHAR(36) | AI 行为统一审计（合并自 prompt_audit + tool_audit_log） | 🆕 合并表 |
| 7 | `assistant_evaluation` | case-service | UUID | AI 助手双轨评分（归属修正） | ✅ 保留 |
| 8 | `kb_category` | kb-service | SERIAL | 分类树（198 节点），全局分类枢纽 | ✅ 保留 |
| 9 | `kbd_entry` | kb-service | BIGSERIAL | KBD 知识条目（整条 embedding，无分块） | ✅ 保留 |
| 10 | `sop_document` | kb-service | SERIAL | SOP 文档（~20,000 字/个） | ✅ 已实现 |
| 11 | `sop_chunk` | kb-service | SERIAL | SOP 分块检索 | ✅ 已实现 |
