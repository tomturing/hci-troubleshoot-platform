...
## D-026：Actions job 展示名漂移导致服务重复补偿发布

**触发场景**：发布基线解析器通过 GitHub Jobs API 的 `name` 判断某服务是否完成构建或晋级，随后 workflow 重构只修改了 job 展示名，没有同步解析器与测试。

**症状**：服务基线永久缺失；任意无关 `main` push 都把该服务加入补偿构建矩阵；每次构建产生新 digest 和新 promotion PR，形成 PR 风暴。单纯合并其中一个 digest PR 不能结束循环。

**根因**：把面向人的可变文案当成机器协议，同时没有跨文件契约测试。进一步地，如果用“PR 已创建”冒充“环境已部署”，即使名称修复也会污染发布状态。

**正确做法**：

1. Jobs API 必须使用明确标注为稳定协议的机器名，并通过测试直接读取 workflow 校验一致性；
2. 分开记录制品已构建、晋级请求已持久化、ArgoCD 已部署三个状态；
3. 服务级基线按各自 job 结果恢复；缺失服务基线必须回退到全局发布基线比较实际构建输入，不能要求无关矩阵服务和整轮 workflow 同时成功，也不能把未知状态无条件扩散成全量重建；
4. promotion 幂等键使用 `service+environment`，新制品更新同一候选并关闭旧候选；
5. 无关路径变化不得触发服务构建，构建输入选择必须有负向回归测试。

**参考案例**：2026-08-18 hci-sim 的旧 `校验 hci-sim immutable GitOps digest` 名称与新自动钉入 job 不一致，导致 #813、#814、#816 连续生成。
