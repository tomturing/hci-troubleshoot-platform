...
- 前端任何组件中不再包含 `crypto.randomUUID` 的直接调用（统一通过 `generateUUID()` 工具函数）
  - 2026-08-19：清理最后残留 `frontend/admin/src/views/KbdReviewView.vue` 的 `createSignalId()`，改用 `generateUUID()`（原实现虽带 `'randomUUID' in crypto` 探测，仍属组件内重复实现降级逻辑，违反 J-1 DRY 原则），至此全前端清零。
