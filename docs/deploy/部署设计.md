...
| 1.18 | 2026-04-27 | **prod 环境独立 HTTPS 入口点**：添加 `prod-websecure` 入口点（端口 3443），支持 prod 与 staging 环境隔离。Ingress 模板新增 `ingress.entrypoint` 字段支持自定义入口点。Traefik HelmChartConfig 和 Service 新增 3443 端口配置。 |
