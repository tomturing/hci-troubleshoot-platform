...
| 1.14 | 2026-04-26 | **Ingress TLS 自动入口切换（PR #227）**：ingress 模板根据 `ingress.tls` 配置自动选择 Traefik 入口点。启用 TLS 时使用 `websecure`（HTTPS，端口 4443），否则使用 `web`（HTTP，端口 4888）。解决公网 HTTP 页面访问 localhost 被 PNA 阻止的问题（详见 [D-007](pitfalls/k8s.md)）。 |
...
*文档版本: 1.13 | 更新日期: 2026-04-17 | AI 助手选择器智能显示配置（PR #171）*
...
| 版本 | 日期 | 改动说明 |
|------|------|----------|
| v1.14 | 2026-04-25 | api-gateway Helm Deployment 补充缺失的 `DATABASE_URL` 环境变量注入（与 case-service 保持一致，使用 `urlquery` 密码编码），修复 K8s 环境下 TerminalService 无法连接数据库导致 `/api/terminal/operations` 500 的问题（PR #216）|
