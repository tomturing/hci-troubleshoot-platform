...
### 12.8 Docker CACHEBUST 参数修复 shared 目录缓存问题（2026-05-20，PR #306）
...
- GitHub Actions checkout 使用 `fetch-depth: 1`，可能影响缓存计算

**修复方案**：

| 文件 | 改动 | 说明 |
|------|------|------|
| `backend/*/Dockerfile`（5 个服务）| 新增 `ARG CACHEBUST=1` | 在 `COPY shared` 之前定义缓存破坏参数 |
| `.github/workflows/ci.yml` | 新增 `build-args: CACHEBUST=${{ github.sha }}` | 每次构建传入不同的 SHA 值 |
...
**效果**：每次构建都传入不同的 `GITHUB_SHA`，Docker 会重新计算 layer hash，强制重新执行 `COPY shared` 步骤。
