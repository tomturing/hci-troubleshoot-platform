...
- `templates/langfuse.yaml`：当 `langfuse.init.enabled=true` 时，在 `langfuse-server` 容器注入：
  - `LANGFUSE_INIT_ORG_ID` / `LANGFUSE_INIT_ORG_NAME` / `LANGFUSE_INIT_ORG_PK` / `LANGFUSE_INIT_ORG_SK`
  - `LANGFUSE_INIT_PROJECT_ID` / `LANGFUSE_INIT_PROJECT_NAME`
  - `LANGFUSE_INIT_PROJECT_PK` / `LANGFUSE_INIT_PROJECT_SK`
...
### 关键约定：project key 复用
`LANGFUSE_INIT_PROJECT_PK` / `LANGFUSE_INIT_PROJECT_SK` **直接取 `.Values.langfuse.publicKey` / `.Values.langfuse.secretKey`**，即 agent-service SDK 注入的同一对 key。二者在模板层绑定、永不不一致，因此 server 一旦建好 project，agent-service SDK 立即可以写入，无需任何手动对齐。
