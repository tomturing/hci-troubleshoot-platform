...
*文档版本: 1.7 | 更新日期: 2026-04-16 | db-migrate hook 死锁修复（§9）*
...
具体 Helm 表达式：
```yaml
name: db-migrate-{{ .Values.dbMigrate.image.tag | default "latest" | regexReplaceAll "[^a-z0-9A-Z-]" "" | lower | trunc 20 | trimSuffix "-" }}
```
...
| | 修改前 | 修改后 |
|---|---|---|
| delete-policy | `BeforeHookCreation` | `BeforeHookCreation,HookSucceeded` |
| 效果 | 成功 Job 残留；失败 Job finalizer 死锁时 BeforeHookCreation 失效 | 成功后自动删除，保持集群整洁；失败时 BeforeHookCreation 仍可清理 |
