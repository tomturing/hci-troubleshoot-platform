...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| **1.61** | **2026-07-14** | **ArgoCD Hook 失败清理 + DB 密码校验修复 + GitOps 目录重组（PR #543）**：① D-011：为 `db-migrate`/`db-seed`/`smoke-test` 三个 ArgoCD Hook 的 `hook-delete-policy` 补充 `HookFailed`，防止 PreSync/PostSync 失败后残留 Job 阻塞后续 re-sync；② 修复 `db-password-check` 中 `$(POSTGRES_PASSWORD)` 在 K8s env.value 不展开的问题，改用 `PGPASSWORD` 环境变量 + 显式 `-h/-U/-d` 参数，避免密码含特殊字符时 URL 编码失败；③ `hci-platform-obs/secrets.yaml` 为 feishu-webhook receiver 增加条件渲染 `{{- if .Values.secrets.feishuWebhookUrl }}`，防止未配置飞书时渲染空 receiver；④ `env-repo-template/staging/values.yaml` 修正镜像仓库名 `hci-<service>` → `<service>`，对齐 CI 实际推送路径，避免 ImagePullBackOff；⑤ `gitops/README.md` 重构 ArgoCD Application 目录为 `local/`（dev）与 `cloud/`（staging/prod）两级，文档与实际目录结构同步。 |
| **1.60** | **2026-07-14** | **KBD LLM 模型默认切换为 kimi-k2.5 并新增思维链统一开关（PR #542）**：① `kb-service` 的 `classifyModel` / `visionModel` Helm 默认值由 `qwen3.7-plus` 切换为 `kimi-k2.5`；② ConfigMap 新增 `LLM_ENABLE_THINKING`（默认 `false`）与 `VISION_GLOBAL_CONCURRENCY`（默认 `3`）注入，`classify.py` 与 `vision_processor.py` 统一读取 `LLM_ENABLE_THINKING` 控制思维链开关，避免隐藏思考 token 拖慢视觉调用；③ 修正 ConfigMap 过时注释（`VISION_MODEL` 实际已由 `kbService.visionModel` 经 ConfigMap 注入）。`hci-platform-env` dev 环境同步将 `visionModel` 设为 `kimi-k2.5`（PR #32）。 |
| **1.59** | **2026-07-10** | **为 kb-service 补充 externalDns 配置支持（PR #525）**：在 `kb-service/deployment.yaml` 模板中新增 `dnsPolicy: None` 和 `dnsConfig` 配置块，通过 `kbService.externalDns` 参数控制。启用后同时保留 K8s 内部 DNS（kube-dns）和公网 DNS（114.114.114.114、1.2.4.8），解决调用外部 Vision API 时 DNS 解析到 Clash Fake-IP (198.18.0.x) 导致的长连接/大包数据挂起超时问题。`values.yaml` 默认值为 `false`，`dev` 环境在 `hci-platform-env` 仓库设置为 `true`。 |
| **1.58** | **2026-07-09** | **实现 kbd data-pipeline Job 模板并修复环境路径（PR #513）**：新增 `kbd-pipeline-job.yaml` 模板以映射所需的 `DATABASE_URL`、`KB_SERVICE_URL` 和 `INTERNAL_API_TOKEN`，解决脚本报错问题。修复 `seed_categories.py` 中遗留的 `.env` 配置文件路径引用。 |
| **1.57** | **2026-07-09** | **强制 GitOps 部署同步以覆盖手动 API Key 修改（PR #511）**：在 `kb-service` 部署模板中强制由 GitOps 部署同步覆盖任何手动对 API key 进行的修改，确保配置一致性。 |
| **1.56** | **2026-07-09** | **修复 push 事件触发时 changes 和 agent reliability 任务失败的问题（PR #510）**：修复 CI 配置文件中由于 push 事件引发的 changes 任务和智能体可靠性回归任务执行失败的问题。 |
| **1.55** | **2026-07-09** | **修复 Langfuse Worker 镜像 tag 遗漏（PR #506）**：PR #505 仅修复 server 镜像，遗漏了 worker 镜像。补齐 worker 镜像 tag 从 \`v3.202.1\` 改为 \`langfuse/langfuse-worker:3\` |
| **1.54** | **2026-07-09** | **修复 Langfuse 镜像 tag 格式错误（PR #505）**：v3.202.1 镜像不存在（Langfuse 镜像版本格式为 \`3.x.x\` 无 \`v\` 前缀），回滚到浮动 tag \`langfuse/langfuse:3\` |
| **1.53** | **2026-07-09** | **Langfuse 镜像版本锁定（PR #503）**：将 `hci-platform-obs` Chart 中 langfuse server/worker 镜像从浮动 tag `:3` 升级并锁定为 `v3.202.1` OSS 版本。避免浮动 tag 漂移带来的不可控升级风险，确保 dev 环境启用 langfuse 链路追踪时版本可追溯。配套在 `hci-platform-env` dev values 中启用 langfuse 配置。**注意**：PR #505 已回滚此变更，原因是 \`v3.202.1\` 镜像不存在 |
| **1.52** | **2026-07-09** | **修复跨命名空间可观测性内嵌控制台加载失败（PR #502）**：回滚 PR #499/500/501 引入的无效跨命名空间服务代理。方案调整为在 `hci-observability` 命名空间 Ingress (`grafana-ingress` / `langfuse-ingress`) 模板中直接通过 annotations 绑定多入口点 (`websecure,web`)，支持从 Admin UI 隔离管理端口 (4888/3888) 直接访问可观测性服务并正确链入 stripPrefix 和 headers 中间件，彻底解决 "no available server" 问题。详见 [可观测性内网隔离失效复盘](events/2026-07-09-可观测性内嵌控制台内网隔离失效修复.md) |
| **1.51** | **2026-07-09** | **使用无 selector Service + Endpoints 替代 ExternalName（PR #501）**：修复 Traefik 不支持 ExternalName Service 的问题。ExternalName Service 在 Traefik 场景下无法作为 Ingress backend 正常工作，请求被路由到 admin-ui 而非 Grafana/Langfuse。解决方案：① 使用无 selector Service（Headless，`clusterIP: None`）；② 手动创建 Endpoints 指向 hci-observability namespace 的服务 IP（Grafana: `10.43.80.87`，Langfuse: `10.43.204.157`）。符合 Kubernetes 跨命名空间服务发现最佳实践 |
| **1.50** | **2026-07-09** | **使用 ExternalName Service 代理跨命名空间可观测性服务（PR #500）**：修复 PR #499 引入的 Kubernetes Ingress 规范违规问题。Ingress 的 `service.name` 字段不支持跨命名空间 FQDN（如 `grafana.hci-observability.svc.cluster.local`），必须符合 DNS-1035 标签规范（仅字母、数字、`-`）。解决方案：① 新增 `observability-services.yaml` 定义 ExternalName Service（`grafana-observability`、`langfuse-observability`）代理跨命名空间服务；② Ingress 引用本地服务名。符合 Kubernetes 最佳实践，实现跨命名空间服务路由 |
