...
| **1.69** | **2026-09-01** | **hci-sim 第八个迁移的 Helm 门禁对齐**：Helm 单元测试显式验证 `000008_unified_version_contract.sql` 已打包，并将稳定迁移文件数更新为 8，修复 main 全量 Helm 校验对 staging 晋级的阻断。 | [hci_sim 独立数据库隔离方案](../solution/hci-sim/events/hci_sim独立数据库隔离方案.md) |
| **1.68** | **2026-08-31** | **hci-sim 第七个迁移的 Helm 门禁对齐**：Helm 单元测试显式验证 `000007_qkv_task_template_v2.sql` 已打包，并将稳定迁移文件数更新为 7，修复 main 全量 Helm 校验失败对 staging 晋级的阻断。 | [hci_sim 独立数据库隔离方案](../solution/hci-sim/events/hci_sim独立数据库隔离方案.md) |
| **1.67** | **2026-08-21** | **staging 环境启用 hci-sim 仿真运行时与控制面**：在 staging 环境新增 `hci-sim-staging` ArgoCD Application，配置独立 `hci_sim` 数据库 PreSync 迁移与网关控制面代理链路，打通 Admin UI Bundle 工厂。 | [hci_sim 独立数据库隔离方案](../solution/hci-sim/events/hci_sim独立数据库隔离方案.md) |
| **1.66** | **2026-08-14** | **后端镜像 Python 依赖源收敛**：后端服务 Dockerfile 不再硬编码单一第三方 PyPI 镜像，默认使用官方 PyPI 安装锁定依赖，避免镜像同步延迟造成新发布版本无法解析；如需内网镜像，必须通过构建环境统一配置，并确保依赖解析与镜像构建使用同一数据源。 | §12.10 |
| **1.63** | **2026-08-10** | **hci_sim 独立数据库阶段二 PR-A**：新增独立 database/角色/迁移 Hook；默认关闭，环境必须显式启用并提供 migrator/runtime 密码。迁移只挂载 `database/hci-sim-migrations`，不读取主库 `desired_schema.sql`；存量表复制和旧表清理仍需观察窗口与独立 contract。 | [hci_sim 独立数据库隔离方案](../solution/hci-sim/events/hci_sim独立数据库隔离方案.md) |
| **1.65** | **2026-08-18** | **hci_sim 迁移打包与执行闭环修复**：Helm PreSync ConfigMap 自动包含 `database/hci-sim-migrations/*.sql` 全部迁移，Job 按文件名顺序逐个幂等执行；CI 增加渲染产物校验，防止应用已读取新列而集群只执行首个迁移。 | [hci_sim 独立数据库隔离方案](../solution/hci-sim/events/hci_sim独立数据库隔离方案.md) |
| **1.65** | **2026-08-13** | **kb-service Catalog 基线持久化 + 运行时代码完整性防护兼容**：Catalog 基线（acli_command_catalog.json / resolution_catalog.json）原打包在镜像层 `/app/shared/resolution/catalogs/`，页面新增/修改仅写入容器可写层，pod 重建后被镜像层覆盖导致回滚。修复：新增 `kbService.persistence`（默认开启）挂载 PVC 到**非受保护路径** `/data/catalogs`（避开 D-015/D-020 禁止覆盖 `/app/shared/` 的 `ValidatingAdmissionPolicy`）；initContainer 首次从镜像拷贝基线（仅缺失时拷贝，保护已有页面修改），主容器经 `ACLI_CATALOG_PATH` / `RESOLUTION_CATALOG_PATH` 环境变量读取持久化目录。同时修复 agent-service 与 kb-service 因 shared 依赖版本漂移导致的 KBD 23821 `tool_contract_revision` stale（CI 构建计划增加 shared→全部后端服务的回归测试）。 | [kb-service Catalog 持久化与版本漂移修复](../deploy/events/2026-08-13-kb-service-catalog-persist-and-drift-fix.md) |
...
**Python 依赖源约束（2026-08-14）**：

- 后端服务 Dockerfile 默认执行 `uv pip install --system --no-cache -r requirements.txt`，使用官方 PyPI，不在镜像构建脚本中硬编码单一第三方镜像地址。
- `uv.lock`、`requirements.txt` 的生成与 Docker 镜像安装必须使用一致的软件包数据源。第三方镜像存在同步延迟时，可能出现锁文件已引用新版本、镜像构建却提示“没有该版本”的假性依赖冲突。
- 隔离网络确需私有或区域镜像时，应由 CI/构建环境统一注入索引配置，并保证镜像已完整同步锁文件涉及的版本；切换前需以干净缓存完成全部后端镜像构建验证。
...
langfuse:
  ...
  init:
    ...
    userName: "admin"
    # 安全提示：请勿在文档中写死真实密码。生产环境应通过 sealed-secret 注入，开发环境也应从 .env 读取。
    userPassword: "${LANGFUSE_INIT_PASSWORD}"
```
...
`terminal-bridge-allow-required` 的入站 `namespaceSelector` 必须匹配 Ingress 网关所在命名空间（K3s/Traefik 部署在 `kube-system`，使用 `app.kubernetes.io/name: traefik`），出站 `podSelector` 必须包含 `matchLabels`：DNS 使用 `k8s-app: kube-dns`（`kube-system`），hci-sim 使用 `app.kubernetes.io/name: hci-sim`（按环境配置为 `hci-sim-dev` 或 `hci-sim-staging`），Tempo 使用 `app.kubernetes.io/name: tempo`（`hci-observability`）。直接把标签键放在 `podSelector` 下会渲染为无选择条件或被 Kubernetes 忽略，扩大网络边界；渲染和 Helm unit test 是发布门禁。
