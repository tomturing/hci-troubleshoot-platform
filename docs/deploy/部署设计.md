...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| **1.53** | **2026-07-09** | **Langfuse 镜像版本锁定（PR #503）**：将 `hci-platform-obs` Chart 中 langfuse server/worker 镜像从浮动 tag `:3` 升级并锁定为 `v3.202.1` OSS 版本。避免浮动 tag 漂移带来的不可控升级风险，确保 dev 环境启用 langfuse 链路追踪时版本可追溯。配套在 `hci-platform-env` dev values 中启用 langfuse 配置 |
