...
| 1.15 | 2026-04-26 | **Grafana Ingress TLS 入口切换（PR #229）**：Grafana ingress 模板同步支持 TLS 入口点自动切换。解决启用 TLS 后 HTTPS 请求无法匹配 Grafana Ingress，被主站 `/` 回退规则捕获显示 customer-ui 的问题（详见 [PIT-036](pitfalls/grafana.md)）。 |
