...
*文档版本: 1.6 | 更新日期: 2026-04-16 | 修复 Tempo→Loki 跳转；固定 Grafana/Loki/Promtail 镜像版本（§8）*

---

## §8 可观测组件镜像版本固定（2026-04-16）

### 8.1 问题

`hci-platform-obs/values.yaml` 中 Grafana/Loki/Promtail 使用 `latest` 标签，导致在镜像更新时意外拉取预发布版本（如 Grafana 13.0.0-pre），引发 Loki 插件 JS 崩溃（`TypeError: Cannot read properties of undefined`）。
...
| 组件 | 旧值 | 新值 |
|------|------|------|
| `grafana.image` | `grafana/grafana:latest` | `grafana/grafana:11.5.2` |
| `loki.image` | `grafana/loki:latest` | `grafana/loki:3.4.2` |
| `promtail.image` | `grafana/promtail:latest` | `grafana/promtail:3.4.2` |

### 8.3 Tempo → Loki 跳转修复

`configs.yaml` 中 `tracesToLogsV2.tags` 原值 `["pod","namespace","app"]` 是 Loki label 名，但 Grafana 要求填写 **span 的 resource attribute key**（提取值后映射到 Loki label）。修复为：

```yaml
tags:
  - key: service.name   # span resource attribute
    value: service_name # 对应 Loki label（由 promtail relabel_configs 映射）
```
