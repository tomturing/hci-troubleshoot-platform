...
| **1.64** | **2026-08-10** | **hci_sim 数据库 bootstrap 凭据加固**：独立迁移 Hook 不再把平台 PostgreSQL 管理密码渲染进 Pod spec 的明文 `env.value`，统一从 `hci-secrets.POSTGRES_PASSWORD` 以 `secretKeyRef` 注入；Helm 单测断言平台、migrator 和 runtime 三项密码均无明文旁路。 | [hci_sim 独立数据库隔离方案](../solution/hci-sim/events/hci_sim独立数据库隔离方案.md) |
