...
## hci-sim K3s 部署边界（2026-07-30）

`hci-sim` 与真实 HCI 双轨部署在独立的 `hci-sim-dev` namespace 中，使用独立的 `deploy/helm/hci-sim` Chart、ClusterIP Service 和不可变 Fixture Manifest。P0 运行时采用 Go 自定义 SSH Server，不启动 OpenSSH、不调用 `/bin/sh -c`；通过签名 Scenario Lease、canonical command fingerprint、bounded worker queue 和 fail-closed 错误 envelope 保证模拟环境只能执行已冻结的 KBD Fixture。

hci-sim 默认仅允许 Terminal Bridge 访问 SSH/2222，Agent Service 不能直连；非必要 Kubernetes API 和内网出站由 NetworkPolicy 阻断。Pod 使用 non-root、read-only root filesystem、drop ALL capabilities、RuntimeDefault seccomp、ResourceQuota 和 LimitRange。stdout/stderr、exit-status、Trace Context 和 fixture hash 通过 Bridge 进入 Artifact、Evaluation、Tempo、Loki 与 Prometheus 查询链路。

当前 dev 已部署 KBD 27123 的 `positive-realistic` Golden Fixture，并完成单场景端到端验收；Windows desktop Bridge compatibility、real/sim differential、20 次稳定性和 100+ 容量梯度仍按验证报告中的后续门禁执行，不因 hci-sim Pod Ready 而提前宣称并发能力。
