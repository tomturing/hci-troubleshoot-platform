...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| **1.48** | **2026-07-02** | **修正 Security Context 中 nginx-conf-d 错误建议（PR #490）**：原 §3 错误地将 `nginx-conf-d` emptyDir 挂载列为 nginx 工作负载的"必须挂载项"，但 `nginx:alpine` 官方镜像在启动时会把 `/etc/nginx/templates/*.template` 经 envsubst 输出到 `/etc/nginx/conf.d/`，emptyDir 覆盖会清空该目录导致 nginx 启动时无 server block → readiness/liveness probe connection refused → CrashLoopBackOff。**正确做法**：只挂载 `/var/cache/nginx` 和 `/var/run` 两个 nginx 真正需要写入的目录，**不要**覆盖 `/etc/nginx/conf.d`。本次同步修正 `admin-ui` 和 `customer-ui` 两个 deployment 模板；详见 [D-009 避坑指南](pitfalls/k8s.md) |
...
# Nginx 类工作负载（如有）额外挂载：
# K3s 下 readOnlyRootFilesystem 导致 nginx 无法写入 /var/cache/nginx 和 /var/run，
# 必须挂载 emptyDir 覆盖这两个路径（PIT-025 规范）。
#
# ⚠️ 注意：/etc/nginx/conf.d 绝对不能用 emptyDir 覆盖！
# nginx:alpine 官方镜像在启动时会把 /etc/nginx/templates/*.template 经 envsubst
# 处理后输出到 /etc/nginx/conf.d/，emptyDir 覆盖会清空该目录导致 nginx 无 server block
# 启动，进而 readiness/liveness probe 全部 connection refused。
# 详见 [D-009 避坑指南](pitfalls/k8s.md)
volumes:
  - name: nginx-cache
    emptyDir: {}
  - name: nginx-run
