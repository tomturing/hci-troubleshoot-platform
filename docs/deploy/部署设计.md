...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| **1.50** | **2026-07-09** | **补齐 admin-ui-ingress Grafana 和 Langfuse 可观测性路径转发（PR #499）**：在 `admin-ui-ingress` 模板中补齐 `/grafana` 和 `/langfuse` 路径转发至跨命名空间服务（`grafana.hci-observability.svc.cluster.local` 和 `langfuse-server.hci-observability.svc.cluster.local`），包括 Langfuse Next.js 应用所需的静态资源路径（`/_next`、`/static`）、认证 API 路径（`/api/auth`、`/api/public`、`/api/trpc`）及精确路径资源（`/icon.svg`、`/wordart-black.svg` 等）。解决通过独立端口（4888）访问 admin-ui 时可观测性页面 iframe 无法加载 Grafana 和 Langfuse 的问题。详见 [Admin UI 内网隔离方案](admin-ui-internal-isolation.md) |
