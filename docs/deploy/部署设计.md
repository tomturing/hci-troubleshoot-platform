...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| **1.44** | **2026-06-13** | **修复 Langfuse 字段错误与 Ingress subpath 路由**：① 修正 MinIO `entrypoint` 为 `command` 字段，修正 ClickHouse `environment` 为 `env`；② 调高 server/worker 内存限额至 2Gi，并配置 `NODE_OPTIONS` 增加 JS 堆内存防止 OOM；③ 新增 `langfuse-stripprefix` 路由重写中间件及专用 Ingress，实现 subpath 下 static/api 的正确重定向与转发；④ 新增 `langfuse-headers` 中间件清除 `X-Frame-Options` 与 `Content-Security-Policy` 安全响应头，解决内嵌式 iframe 拒绝连接的 clickjacking 拦截问题；⑤ 在 Ingress 中补齐 `/api/trpc` 路由规则，解决 Langfuse 前后端 tRPC 通信接口被平台主 API Gateway 误拦截劫持导致的 'Unable to transform response from server' 报错 |
